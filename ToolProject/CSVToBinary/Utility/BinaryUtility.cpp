...
const uint BinaryUtility::mWCharType = (uint)typeid(wchar_t).hash_code();
...
const uint BinaryUtility::mLLongType = (uint)typeid(llong).hash_code();
...
const uint BinaryUtility::mLLongsType = (uint)typeid(llong*).hash_code();
const uint BinaryUtility::mULLongsType = (uint)typeid(ullong*).hash_code();
const uint BinaryUtility::mBoolListType = (uint)typeid(Vector<bool>).hash_code();
const uint BinaryUtility::mCharListType = (uint)typeid(Vector<char>).hash_code();
const uint BinaryUtility::mByteListType = (uint)typeid(Vector<byte>).hash_code();
const uint BinaryUtility::mShortListType = (uint)typeid(Vector<short>).hash_code();
const uint BinaryUtility::mUShortListType = (uint)typeid(Vector<ushort>).hash_code();
const uint BinaryUtility::mIntListType = (uint)typeid(Vector<int>).hash_code();
const uint BinaryUtility::mUIntListType = (uint)typeid(Vector<uint>).hash_code();
const uint BinaryUtility::mFloatListType = (uint)typeid(Vector<float>).hash_code();
const uint BinaryUtility::mLLongListType = (uint)typeid(Vector<llong>).hash_code();
const uint BinaryUtility::mULLongListType = (uint)typeid(Vector<ullong>).hash_code();
...
// 计算 16进制的c中1的个数
constexpr uint BinaryUtility::crc_check(char c)
{
	uint count = 0;
...
bool BinaryUtility::readBuffer(const char* buffer, uint bufferSize, uint& index, char* dest, uint destSize, uint readSize)
{
