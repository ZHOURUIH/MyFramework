...
public:
	Map() = default;
	explicit Map(initializer_list<pair<const Key, Value>> _Ilist)
	{
		mMap.insert(_Ilist);
	}
	virtual ~Map() { clear(); }
	...
	uint valueToListFilter(Array<Length, Value>& valueList, const Value& exceptValue, uint startIndex = 0) const
	{
	...
	template<size_t Length>
	uint valueToList(Array<Length, Value>& valueList, uint startIndex = 0) const
	{
		if (startIndex >= mMap.size())
		{
			return 0;
