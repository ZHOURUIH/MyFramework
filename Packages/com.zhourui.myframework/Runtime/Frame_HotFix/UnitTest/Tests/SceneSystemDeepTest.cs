...
// SceneSystem 深度测试
// 聚焦多场景可见性状态机(showScene/hideScene)与注册回调链的复杂交互语义,
// 而非只测"调用某函数不报错"。
//
// 设计要点:
//   - SceneSystem 的真实 showScene/hideScene 通过遍历 mSceneList 调用
//     scene.setActive / onShow / onHide, 不接触 SceneManager(纯逻辑)。
//   - MockScene 用真实根节点(GameObject)承载 base.setActive/getActive(它们非虚,
//     不能 override), 仅 override 虚方法 onShow/onHide/update 记录调用轨迹;
//     经反射注入 mSceneList, 从而在 EditMode 无真实场景/资源下深测可见性状态机。
//   - 不测: loadSceneAsync/unloadScene/unloadSceneOnly/setMainScene(依赖
//     ResourceManager / SceneManager.LoadSceneAsync/UnloadSceneAsync, 触发真实场景加载日志)。
//   - 每个用例 finally 里先反射清空 mSceneList 再 destroy() 局部 SceneSystem:
//     SceneSystem.destroy() 会遍历 mSceneList 调 unloadSceneOnly → 
//     SceneManager.UnloadSceneAsync(未加载场景名) 会产生 error 日志,
//     清空列表即可绕过, 且避免 mSceneList/mSceneRegisteList 相互污染。
public static class SceneSystemDeepTest
{
