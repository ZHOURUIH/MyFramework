...
// Curve 深度测试 — 曲线系统的复杂调用链
// 覆盖: KEY_CURVE 常量注册表 / KeyFrameManager.loadAllCalculatedCurve 的批量注册 /
//       getKeyFrame 的 ID→曲线解析 / TweenUtility.EvaluateCurve 桥接 /
//       ComponentKeyFrame 的关键帧求值时序(mCurrentTime*mInverseOnceLength) / 
//       Back/Elastic/Bounce 振荡曲线的过冲(overshoot)与回弹特性
//
// 与既有 CurveTest 的区别:
//   CurveTest 只对每个曲线单点调用 evaluate, 验证端点/中点/单调性;
//   本深度测试验证的是"曲线如何被框架核心系统(KeyFrameManager/TweenUtility/ComponentKeyFrame)消费"的
//   复杂调用链是否正确, 以及振荡曲线(Back/Elastic/Bounce)在真实动画中产生的过冲/回弹行为是否达预期。
//
// 可测性设计: 每个测试方法都用局部 `new KeyFrameManager()` 实例(构造时会把内置曲线注册进
//   自身 mCurveList), 不依赖全局单例 mKeyFrameManager, 也不触发 init() 创建 GameObject,
//   完好用 try-finally destroy() 清理局部实例。loadAllCalculatedCurve 用独立临时字典调用, 不污染全局注册表。
public static class CurveDeepTest
{
