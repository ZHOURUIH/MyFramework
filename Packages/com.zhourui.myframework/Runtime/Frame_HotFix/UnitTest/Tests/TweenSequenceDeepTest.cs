...
// TweenSequence 深度测试 —— 补间序列的时间轴推进与状态转换链
// 与 TweenSequenceTest/TweenGroupTest(单接口/纯逻辑) 不同, 本文件覆盖单接口调用测不到的
// 复杂时间链行为, 全部基于真正调用 evaluateSequence 驱动时间推进:
//   1. buildTimeline: play() 按 startDelay 正确分配每个启用轨道的时间窗口
//   2. 时间推进: 多时间点 evaluateSequence 下轨道 未开始→播放中→结束 的完整状态机
//   3. 最后一帧钉扎: 轨道结束后保证补间定格在终点值
//   4. 串行组内多轨道: 前一轨道结束后下一轨道才接力播放
//   5. 禁用轨道: 不占用时间轴, 不参与推进
//   6. SELF 起始值: play() 时快照节点当前值, 之后移动节点不改起始值
//   7. SNAPSHOT/REALTIME 目标值: 分别快照/实时读取目标节点
//   8. stop() 复位: mResetWhenStop/forceReset 恢复 play() 时记录的原始 transform
// 曲线全部用内置直线 CurveZeroOne(duration 归一化), 使时间↔进度映射可精确断言。
public static class TweenSequenceDeepTest
{
