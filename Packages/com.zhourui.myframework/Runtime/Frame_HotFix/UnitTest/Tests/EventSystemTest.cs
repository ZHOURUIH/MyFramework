...
public static class EventSystemTest
{
	...
	class TestEvent_Deep : GameEvent { }
	class TestEvent2_Deep : GameEvent { }
	// 带字段的事件,用于测试带参事件的数据流
	class TestEventWithValue_Deep : GameEvent
	{
		public int value;
		public override void resetProperty()
		{
	...
	class TestListener_Deep : IEventListener { }
	...
	private static void testUnlistenSelfDuringDispatch()
	{
		...
		var l1 = new TestListener_Deep();
		var l2 = new TestListener_Deep();
		int c1 = 0, c2 = 0;
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			c1++;
			sys.unlistenEvent(l1); // 回调中移除自己
		}, l1);
		sys.listenEvent<TestEvent_Deep>(_ => c2++, l2);

		sys.pushEvent<TestEvent_Deep>();
	...
	private static void testMultipleListenersRemoveOneDuringDispatch()
	{
		...
		var l1 = new TestListener_Deep();
		var l2 = new TestListener_Deep();
		var l3 = new TestListener_Deep();
		int c1 = 0, c2 = 0, c3 = 0;
		// l2 的回调移除 l3(l2 之后的监听者)
		sys.listenEvent<TestEvent_Deep>(_ => c1++, l1);
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			c2++;
			sys.unlistenEvent(l3);
		}, l2);
		sys.listenEvent<TestEvent_Deep>(_ => c3++, l3);
	...
	private static void testListenSameEventTwiceUnlistenOnce()
	{
		...
		var l1 = new TestListener_Deep();
		int count = 0;
		sys.listenEvent<TestEvent_Deep>(_ => count++, l1);
		sys.listenEvent<TestEvent2_Deep>(_ => count++, l1);

		sys.pushEvent<TestEvent_Deep>();
		sys.pushEvent<TestEvent2_Deep>();
		assertEqual(2, count, "两个事件各触发1次");

		// 只移除 TestEvent_Deep 的监听, TestEvent2_Deep 保留
		sys.unlistenEvent<TestEvent_Deep>(l1);
		sys.pushEvent<TestEvent_Deep>();
		sys.pushEvent<TestEvent2_Deep>();
		assertEqual(3, count, "TestEvent_Deep 不再触发, TestEvent2_Deep 触发第2次");
	}
	...
	private static void testUnlistenSelfDuringCharacterDispatch()
	{
		...
		int c1 = 0, c2 = 0;
		long charID = 88881;
		sys.listenEvent<TestEvent_Deep>(charID, _ =>
		{
			c1++;
			sys.unlistenEvent(l1); // 角色回调中移除自己
		}, l1);
		sys.listenEvent<TestEvent_Deep>(charID, _ => c2++, l2);

		sys.pushEvent<TestEvent_Deep>(charID);
		assertEqual(1, c1, "l1 收到1次");
		assertEqual(1, c2, "l2 仍收到1次(不受 l1 unlisten 影响)");
	...
	private static void testUnlistenInCharacterAlsoAffectsGlobal()
	{
		...
		long charID = 88883;
		sys.listenEvent<TestEvent_Deep>(charID, _ => charCount++, l1);
		sys.listenEvent<TestEvent_Deep>(_ => globalCount++, l1);
	...
	private static void testListenNewDuringDispatchNotFiredThisTime()
	{
		var sys = new EventSystem();
		var l1 = new TestListener_Deep();
		var lNew = new TestListener_Deep();
		int c1 = 0, cNew = 0;
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			c1++;
			sys.listenEvent<TestEvent_Deep>(_ => cNew++, lNew); // 派发中新增监听者
		}, l1);
	...
	private static void testUnlistenThenRelistenDuringDispatch()
	{
		...
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			...
			sys.listenEvent<TestEvent_Deep>(_ => count++, l1); // append 新槽
		}, l1);
	...
	// ═════════════════════════════════════════════════════════════════
	// 嵌套派发: TestEvent_Deep 回调里 pushEvent 另一个事件 TestEvent2_Deep
	...
	{
		var sys = new EventSystem();
		var lA = new TestListener_Deep();
		var lB = new TestListener_Deep();
		var lC = new TestListener_Deep();
		int cA = 0, cB = 0, cC = 0, cInner = 0;
		// lA 收到 TestEvent_Deep 后嵌套 push TestEvent2_Deep; 内层 lA 也监听 TestEvent2_Deep
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			cA++;
			sys.pushEvent<TestEvent2_Deep>(); // 嵌套派发另一事件
		}, lA);
		sys.listenEvent<TestEvent_Deep>(_ =>
		{
			cB++;
		...
		sys.listenEvent<TestEvent_Deep>(_ => cC++, lC);
		// 内层 TestEvent2_Deep 的监听者
		sys.listenEvent<TestEvent2_Deep>(_ => cInner++, lA);

		// 外层派发 TestEvent_Deep
		sys.pushEvent<TestEvent_Deep>();
		// 外层: lA 收到并嵌套 push TestEvent2_Deep → lA 的 TestEvent2_Deep 回调 cInner 收到1次
		assertEqual(1, cA, "lA 收到1次");
		assertEqual(1, cInner, "嵌套派发的 TestEvent2_Deep 被 lA 收到1次");
		...
		assertEqual(2, cInner, "嵌套 TestEvent2_Deep 收到2次");
	...
	private static void testUnlistenThenCompactReuseSlot()
	{
		...
		int outside = 0;
		var keeper = new TestListener_Deep();
		sys.listenEvent<TestEvent_Deep>(_ => outside++, keeper);

		// 连续 10 次: 新增临时监听者, 派发中移除自己, 确保不崩溃且临时者不残留
		for (int i = 0; i < 10; ++i)
		{
			var temp = new TestListener_Deep();
			int tempCount = 0;
			sys.listenEvent<TestEvent_Deep>(_ =>
			{
				tempCount++;
	...
	private static void testRemoveCharacterEventDuringDispatch()
	{
		...
		sys.listenEvent<TestEvent_Deep>(charID, _ => cB++, lB);
		sys.listenEvent<TestEvent_Deep>(_ => g++, lG); // 全局监听
	...
	private static void testRemoveCharacterEventAffectsOnlyThatCharacter()
	{
		...
		sys.listenEvent<TestEvent_Deep>(charID1, _ => c1++, l1);
		sys.listenEvent<TestEvent_Deep>(charID2, _ => c2++, l2);

		// 只移除角色1, 角色2不受影响
		sys.removeCharacterEvent(charID1);

		sys.pushEvent<TestEvent_Deep>(charID1);
		assertEqual(0, c1, "角色1监听已移除");
		sys.pushEvent<TestEvent_Deep>(charID2);
	...
	private static void testRemoveCharacterEventThenUpdateCleansUp()
	{
		...
		for (int i = 0; i < 10; ++i)
		{
			var l = new TestListener_Deep();
			int c = 0;
			sys.listenEvent<TestEvent_Deep>(charID, _ => c++, l);
			sys.pushEvent<TestEvent_Deep>(charID);
	...
	private static void testCharacterAddNewDuringDispatch()
	{
		...
		{
			cA++;
			sys.listenEvent<TestEvent_Deep>(charID, _ => cNew++, lNew);
		}, lA);
	...
	private static void testUnlistenTypeSpecificDuringCharacter()
	{
		...
		int c1 = 0, c2 = 0;
		long charID = 77776;
		sys.listenEvent<TestEvent_Deep>(charID, _ => c1++, l1);
		sys.listenEvent<TestEvent2_Deep>(charID, _ => c2++, l1);

		sys.pushEvent<TestEvent_Deep>(charID);
		sys.pushEvent<TestEvent2_Deep>(charID);
		assertEqual(1, c1, "TestEvent_Deep 角色收到1次");
		assertEqual(1, c2, "TestEvent2_Deep 角色收到1次");

		// 只移除 TestEvent_Deep 的角色注册
		sys.unlistenEvent<TestEvent_Deep>(l1);
		sys.pushEvent<TestEvent_Deep>(charID);
		sys.pushEvent<TestEvent2_Deep>(charID);
		assertEqual(1, c1, "TestEvent_Deep 不再触发");
		assertEqual(2, c2, "TestEvent2_Deep 继续触发");
	}
	...
	{
		var sys = new EventSystem();
		var lChar = new TestListener_Deep();
		var lGlobal = new TestListener_Deep();
		int cChar = 0, cGlobal = 0;
		long charID = 77777;
		sys.listenEvent<TestEvent_Deep>(charID, _ =>
		{
			cChar++;
			sys.pushEvent<TestEvent2_Deep>(); // 角色回调里 push 全局事件
		}, lChar);
		sys.listenEvent<TestEvent2_Deep>(_ => cGlobal++, lGlobal); // 全局监听 TestEvent2_Deep
	...
	private static void testListenByTypeIDAndUnlistenDuringDispatch()
	{
		...
		sys.listenEvent(TypeID<TestEvent_Deep>.ID, () =>
		{
			c1++;
			sys.unlistenEvent(l2); // 派发中移除排在其后的 l2
		}, l1);
		sys.listenEvent(TypeID<TestEvent_Deep>.ID, () => c2++, l2);
	...
	private static void testParamSharedBetweenGlobalAndCharacter()
	{
		...
		var lGlobal = new TestListener_Deep();
		var lChar = new TestListener_Deep();
		TestEventWithValue_Deep seenByGlobal = null;
		TestEventWithValue_Deep seenByChar = null;
		long charID = 77778;
		sys.listenEvent<TestEventWithValue_Deep>(e => seenByGlobal = e, lGlobal);
		sys.listenEvent<TestEventWithValue_Deep>(charID, e => seenByChar = e, lChar);

		var evt = CLASS<TestEventWithValue_Deep>();
		evt.value = 555;
	...
	private static void testParamMutationAffectsLaterListeners()
	{
		...
		sys.listenEvent<TestEventWithValue_Deep>(e =>
		{
			seen1 = e.value;
			e.value = 100; // 修改共享对象
		}, l1);
		...
		sys.listenEvent<TestEventWithValue_Deep>(e => seen3 = e.value, l3);
	...
	private static void testParamValueFlowsThroughNestedDispatch()
	{
		...
		var lB = new TestListener_Deep();
		int aSeen = -1, bSeen = -1;
		// lA 收到 TestEventWithValue_Deep 后, 把值传递并嵌套 push 一个 TestEvent2_Deep
		sys.listenEvent<TestEventWithValue_Deep>(e =>
		{
			aSeen = e.value;
			// 嵌套派发 TestEvent2_Deep (无参)
			sys.pushEvent<TestEvent2_Deep>();
		}, lA);
		sys.listenEvent<TestEvent2_Deep>(_ => bSeen = aSeen, lB);
	...
	private static void testParamCallbackOrderObservesMutations()
	{
		...
		sys.listenEvent<TestEventWithValue_Deep>(e => { order.Add(e.value); e.value = e.value + 10; }, l1);
		sys.listenEvent<TestEventWithValue_Deep>(e => { order.Add(e.value); e.value = e.value + 10; }, l2);
		sys.listenEvent<TestEventWithValue_Deep>(e => order.Add(e.value), l3);
