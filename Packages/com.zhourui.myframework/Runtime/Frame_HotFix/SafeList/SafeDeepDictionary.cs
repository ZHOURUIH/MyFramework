...
{
	public struct SafeDeepDictionaryEnumerator : IDisposable
	{
		private SafeDeepDictionary<Key, Value> mOwner;
		private Dictionary<Key, Value>.Enumerator mEnumerator;
		private Dictionary<Key, Value> mReadList;
		public SafeDeepDictionaryEnumerator(SafeDeepDictionary<Key, Value> safeList)
		{
			mOwner = safeList;
	...
	public SafeDeepDictionaryEnumerator GetEnumerator() { return new(this); }
	...
	//------------------------------------------------------------------------------------------------------------------------------
	// 获取用于更新的列表,仅在迭代器中被调用
	private Dictionary<Key, Value> startForeach()
	{
	...
	// 遍历结束后,仅在迭代器中被调用
	private void endForeach(Dictionary<Key, Value> list)
	{
		list.Clear();
