...
{
	protected List<AssetLoadDoneCallback> mCallback = new();// 回调列表
	protected List<string> mLoadPath = new();               // 用于回调传参的加载路径列表,实际上里面都是mResourceName
	...
	public void callbackAll()
	{
		...
		List<AssetLoadDoneCallback> tempCallbackList = new(mCallback);
		List<string> tempLoadPath = new(mLoadPath);
		mCallback.Clear();
		mLoadPath.Clear();
