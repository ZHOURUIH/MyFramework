...
namespace NativeWebSocket
{
  ...
  public class WebSocket : IWebSocket {
    ...
    public Task Send (byte[] data, int dataSize = -1)
    {
      if (dataSize <= 0)
      {
        dataSize = data.Length;
      }
      int ret = WebSocketSend (this.instanceId, data, dataSize);
	...
	public class WebSocket : IWebSocket
    {
        ...
        public Task Send(byte[] bytes, int length = -1)
        {
            if (length < 0)
            {
                length = bytes.Length;
            ...
            return SendMessage(sendBytesQueue, WebSocketMessageType.Binary, new ArraySegment<byte>(bytes, 0, length));
        }
