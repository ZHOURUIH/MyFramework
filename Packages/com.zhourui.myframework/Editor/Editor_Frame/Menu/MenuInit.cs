...
public class MenuInit
{
    ...
    private const string OBFUZ_ASSEMBLY_FRAME_HOTFIX = "Frame_HotFix";													// 需要Obfuz混淆的框架热更程序集
    private const string OBFUZ_ASSEMBLY_HOTFIX = "HotFix";																// 需要Obfuz混淆的业务热更程序集
    ...
    protected static void initHybridCLRObfuz()
    {
        ...
        {
            obfuzChanged = true;
            Debug.Log("已设置Obfuz混淆程序集: " + OBFUZ_ASSEMBLY_FRAME_HOTFIX + ", " + OBFUZ_ASSEMBLY_HOTFIX);
        }
    ...
    protected static bool setObfuzCodeOutputPath(string outputPath)
    {
        ...
        if (codeOutputPath.stringValue == outputPath)
        {
            return false;
        }
    ...
    protected static bool setObfuzAssemblySettings()
    {
        ...
        SerializedProperty assemblySettings = serializedObject.FindProperty("assemblySettings");
        if (assemblySettings == null)
        {
            Debug.LogError("未找到ObfuzSettings.assemblySettings字段");
            return false;
        }

        bool changed = false;

        SerializedProperty obfuscateObfuzRuntime = assemblySettings.FindPropertyRelative("obfuscateObfuzRuntime");
        if (obfuscateObfuzRuntime == null)
        {
            Debug.LogError("未找到ObfuzSettings.assemblySettings.obfuscateObfuzRuntime字段");
        }
        else if (obfuscateObfuzRuntime.boolValue)
        {
            obfuscateObfuzRuntime.boolValue = false;
            changed = true;
        }

        SerializedProperty assembliesToObfuscate = assemblySettings.FindPropertyRelative("assembliesToObfuscate");
        if (assembliesToObfuscate == null)
        {
            Debug.LogError("未找到ObfuzSettings.assemblySettings.assembliesToObfuscate字段");
            return changed;
        }
        if (!assembliesToObfuscate.isArray)
        {
            Debug.LogError("ObfuzSettings.assemblySettings.assembliesToObfuscate不是数组或List字段");
            return changed;
        }

        if (addStringToSerializedArray(assembliesToObfuscate, OBFUZ_ASSEMBLY_FRAME_HOTFIX))
        {
            changed = true;
        }
        if (addStringToSerializedArray(assembliesToObfuscate, OBFUZ_ASSEMBLY_HOTFIX))
        {
            changed = true;
        }
    ...
    // 向SerializedProperty字符串数组中添加一个值,已存在则不重复添加
    protected static bool addStringToSerializedArray(SerializedProperty arrayProperty, string value)
    {
        if (arrayProperty == null || !arrayProperty.isArray || value.isEmpty())
        {
            return false;
        }
        ...
        SerializedProperty newItem = arrayProperty.GetArrayElementAtIndex(index);
        if (newItem == null)
        {
            return false;
        }
