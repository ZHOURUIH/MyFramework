...
{
    public const string MENU_NAME = MENU_ROOT_NAME + "检查资源/";
    [MenuItem(MENU_NAME + "查找文件引用  %Q", false, 2)]
	public static void searchReference()
	{
		...
		if (isFileExist(path))
		{
			doSearchReference(path, getAllGuidInverseRefList());
		}
	...
	[MenuItem(MENU_NAME + "查找资源引用了哪些文件", false, 2)]
	public static void searchResourceRefOther()
	{
		...
		if (isFileExist(path))
		{
			doSearchResourceRefOther(path, getAllMeta());
		}
		// 选择的是目录,则查找目录中所有文件的引用
		else if (isDirExist(path))
		{
	...
	[MenuItem(MENU_NAME + "更新文件引用信息缓存", false, 3)]
	public static void refreshFileGUIDLinesCache()
	{
		FileGUIDLinesSerializer.serialize(getAllGuidInverseRefListInternal());
	}
	[MenuItem(MENU_NAME + "更新meta信息缓存", false, 3)]
	public static void refreshMetaCache()
	{
		MetaSerializer.serialize(getAllMetaInternal());
	}
	[MenuItem(MENU_NAME + "查找TPAtlas图集引用", false, 4)]
	public static void checkTPAtlasReference()
	{
		...
		if (isFileExist(path))
		{
			...
			{
				doCheckTPAtlasReference(path, getAllGuidInverseRefList());
			}
		}
		else if (isDirExist(path))
		{
	...
	[MenuItem(MENU_NAME + "检查所有界面是否添加了适配组件", false, 5)]
		...
		else if (isDirExist(path))
		{
			if (checkAll || EditorUtility.DisplayDialog("检查是否添加了适配组件", "确认查找文件夹中所有的界面? " + path, "确认", "取消"))
			{
				...
				foreach (string item in Directory.GetFiles(path, "*.prefab", SearchOption.AllDirectories))
				{
					...
					validFiles.addIf(file, !file.endWith(".meta", false) && !FrameEditorSettings.getInstance().IgnoreScaleAnchorCheck.contains(fileName));
				}
				// 开始查找所有文件的引用
				int count = validFiles.Count;
				for (int i = 0; i < count; ++i)
	...
	[MenuItem(MENU_NAME + "检查未引用的资源", false, 99)]
	public static void checkUnusedResources()
	{
		...
		if (isFileExist(path))
		{
			doCheckUnusedFile(path, getAllGuidInverseRefList());
		}
		else if (isDirExist(path))
		{
	...
	[MenuItem(MENU_NAME + "检查选中对象的原点是否在模型底部", false, 100)]
	...
	[MenuItem(MENU_NAME + "检查材质引用丢失", false, 101)]
	public static void checkMaterialMissingReference()
	{
		...
		{
			FileRefGUIDs fileInfo = item.Value;
	...
	[MenuItem(MENU_NAME + "检查材质贴图是否存在", false, 102)]
	public static void checkMaterialTextureValid()
	{
		...
		if (isFileExist(path))
		{
			EditorCommonUtility.checkMaterialTextureValid(path, getAllMeta());
		}
		// 选择的是目录,则查找目录中所有文件的引用
		else if (isDirExist(path))
		{
	...
	[MenuItem(MENU_NAME + "检查材质是否引用了shader未使用的贴图", false, 103)]
	public static void checkMaterialTextureReference()
	{
		...
		if (isFileExist(path))
		{
			checkMaterialTexturePropertyValid(path, getAllMeta());
		}
		// 选择的是目录,则查找目录中所有文件的引用
		else if (isDirExist(path))
		{
	...
	[MenuItem(MENU_NAME + "检查热更与非热更资源是否相互引用", false, 105)]
	...
	[MenuItem(MENU_NAME + "检查图集中不存在的图片", false, 108)]
	...
	[MenuItem(MENU_NAME + "检查是否有重复文件", false, 109)]
	...
	[MenuItem(MENU_NAME + "检查预设根节点是否带变换", false, 111)]
	public static void checkPrefabRootTransform()
	{
		...
		for (int i = 0; i < fileCount; ++i)
		{
			...
			if (!prefab.transform.localPosition.isZero() ||
				!prefab.transform.localEulerAngles.isZero() ||
	...
	[MenuItem(MENU_NAME + "检查所有Prefab文件MeshCollider的模型Read-Write", false, 112)]
	...
	[MenuItem(MENU_NAME + "检查所有场景的MeshCollider模型Read-Write", false, 113)]
	...
	[MenuItem(MENU_NAME + "【当前场景】检查场景MeshCollider模型Read-Write", false, 114)]
	...
	[MenuItem(MENU_NAME + "【当前场景】检查场景的Layer空对象", false, 115)]
	...
	[MenuItem(MENU_NAME + "检查资源中是否有代码文件引用丢失", false, 116)]
	...
	[MenuItem(MENU_NAME + "检查布局预制体的缩放值是否符合规范", false, 116)]
	...
	[MenuItem(MENU_NAME + "检查布局中是否有空的Image或Sprite", false, 117)]
	...
	[MenuItem(MENU_NAME + "检查是否使用了内置图片的UI节点", false, 118)]
	...
	[MenuItem(MENU_NAME + "检查资源文件名规范", false, 119)]
	...
	[MenuItem(MENU_NAME + "检查所有UI布局中根节点是否为不保持宽高比的ScaleAnchor", false, 121)]
	...
	[MenuItem(MENU_NAME + "检查所有UI Prefab上的多语言文本组件", false, 125)]
	...
	[MenuItem(MENU_NAME + "查找所有UI Prefab上的多语言文本", false, 126)]
	...
	[MenuItem(MENU_NAME + "检查所有代码中的中文文本", false, 127)]
	...
	[MenuItem(MENU_NAME + "检查所有UI Prefab上的没有添加多语言的带文字图片", false, 128)]
	...
	[MenuItem(MENU_NAME + "检查被单一引用的文件是否与引用其的文件在同一个目录", false, 129)]
	public static void checkBeSingleUsedFileStayWith()
	{
		...
		if (isFileExist(path))
		{
			doCheckSingleUsedFile(path, getAllGuidInverseRefList(), false);
		}
		// 选择的是目录,则查找目录中所有文件的引用
		else if (isDirExist(path))
		{
	...
	// 如果想要能够检查外部对文件的依赖,比如代码中引用的资源,表格中引用的资源,则需要重写此方法,并且传入正确的引用列表方法
	[MenuItem(MENU_NAME + "打开检查资源界面", false, 131)]
	public static void checkResWindows()
	{
		EditorWindow.GetWindow<CheckResourcesWindow>(true, "检查资源", true).start(null);
	}
	[MenuItem(MENU_NAME + "打开提取文本组件界面", false, 131)]
