...
{
	public const string MENU_NAME = MENU_ROOT_NAME + "AssetBundle/";
	...
	public static void refreshSingleAssetBundleName()
	{
		Selection.objects.For(obj =>
		{
			...
			{
				refreshFileAssetBundleName(null, path, isForceSinglePath(projectPathToFullPath(path), FrameEditorSettings.getInstance().ForceSingleFolder));
			}
		});
	}
    ...
    public static bool packAssetBundle(BuildTarget target, string outputPath, bool showMessageBox)
	{
		...
		do
		{
			...
			// 清理不打包的AssetBundle名
			clearUnPackAssetBundleName(findFilesNonAlloc(F_GAME_RESOURCES_PATH), FrameEditorSettings.getInstance().UnpackFolder);
			// 设置bunderName
	...
	{
		if (isUnpackPath(fullPath, FrameEditorSettings.getInstance().UnpackFolder))
		{
			return true;
		}
		...
		bool isForceSingle = isForceSinglePath(fullPath, FrameEditorSettings.getInstance().ForceSingleFolder);
	...
	{
		return FrameEditorSettings.getInstance().KeepFolder.contains(folder => folder == name || folder + ".meta" == name);
	}
	// 清理之前设置的bundleName
