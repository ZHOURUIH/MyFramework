...
public class AssetBundleStudioWindow : GameEditorWindow
{
	...
	private void loadAssetList()
	{
		...
		foreach (Object asset in mAssetBundle.LoadAllAssets())
		{
			mAssetList.Add(new AssetItem
			{
				mAssetName = asset.name,
				mTypeName = asset.GetType().Name,
				mAsset = asset,
			});
		}
		foreach (string scenePath in mAssetBundle.GetAllScenePaths())
		{
			mAssetList.Add(new AssetItem
			{
	...
	private bool isMatchSearch(AssetItem item)
	{
		return mSearchText.isEmpty() ||
			   item.mAssetName.IndexOf(mSearchText, StringComparison.OrdinalIgnoreCase) >= 0 ||
