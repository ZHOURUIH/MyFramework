...
public class RefInfo
{
	...
	public bool mOuterRefCount;				// 是否在其他地方引用,比如代码直接引用,表格引用等
}
...
public class CheckResourcesWindow : GameEditorWindow
{
	...
	protected Func<HashSet<string>> mCollectUsedFileFunc;		// 外部传入函数,用于获取指定文件是否在外部被引用
    public void start(Func<HashSet<string>> collectUsedFileCallback)
    {
		mCollectUsedFileFunc = collectUsedFileCallback;
        Show();
