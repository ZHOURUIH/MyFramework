...
// Spine动画拆分窗口,自动识别当前Runtime支持的JSON或Binary Skeleton源数据,生成最小化基础Skeleton文件。
...
public class SpineAnimationSubsetWindow : EditorWindow
{
    ...
    protected void scanAnimations()
    {
        ...
        try
        {
            ...
            mScanResult = scanSourceData(mSourceBytes, mSourceAssetPath);
            mAnimationItems.Clear();
            for (int i = 0; i < mScanResult.mAnimations.Count; ++i)
            {
            ...
            Debug.Log("Spine动画扫描完成" + "\n文件:" + mSourceAssetPath +
                ...
                "\n基础数据:" + getMemoryText(mScanResult.mAnimationCountPosition) +
    ...
    // 统一Skeleton源数据扫描入口。4.2/4.3支持官方JSON导出，Binary资源仍走各版本Direct Binary Scanner。
    protected static SpineBinaryScanResult scanSourceData(byte[] sourceBytes, string sourceAssetPath)
    {
        if (sourceBytes == null || sourceBytes.Length == 0)
        {
            throw new ArgumentException("Skeleton源数据为空", nameof(sourceBytes));
        }
        if (string.IsNullOrEmpty(sourceAssetPath))
        {
            throw new ArgumentException("Skeleton源资源路径为空", nameof(sourceAssetPath));
        }
#if SPINE_RUNTIME_43
        if (sourceAssetPath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
        {
            return Spine43JsonUtility.scan(sourceBytes);
        }
#elif SPINE_RUNTIME_42
        if (sourceAssetPath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
        {
            return Spine42JsonUtility.scan(sourceBytes);
        }
#elif SPINE_RUNTIME_41
        if (sourceAssetPath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
        {
            return Spine41JsonUtility.scan(sourceBytes);
        }
#endif
        SpineBinaryScannerVersion scanner = new SpineBinaryScannerVersion();
        return scanner.scan(sourceBytes);
    }
