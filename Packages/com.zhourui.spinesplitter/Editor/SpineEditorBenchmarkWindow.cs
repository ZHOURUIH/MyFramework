...
public class SpineEditorBenchmarkWindow : EditorWindow
{
	...
	private void OnGUI()
	{
		...
		GUI.enabled = true;
		EditorGUILayout.Space(8);
		EditorGUILayout.HelpBox("Performance测试正式Reader的文件解析、动画解析和总耗时；Memory测试动画结构；Multi Animation Memory使用正式Reader扫描全部动作；Dynamic Animation LRU Test验证60秒驻留、LRU顺序、Pin、播放保护和Dispose事件自动Trim。", UnityEditor.MessageType.Info);
	}
	private void runPerformanceBenchmark()
	{
		...
		List<double> readerTimes = new(mSampleCount);
		List<double> totalTimes = new(mSampleCount);
		...
		for (int i = 0; i < mSampleCount; ++i)
		{
			...
			readerTimes.Add(readerWatch.Elapsed.TotalMilliseconds);
			totalTimes.Add(totalWatch.Elapsed.TotalMilliseconds);
		}
		long gcAfter = GC.GetTotalMemory(false);
		Debug.Log(buildPerformanceReport(animationBytes.Length, fileTimes, readerTimes, totalTimes, gcAfter - gcBefore));
	}
	...
	private string buildPerformanceReport(int fileBytes, List<double> fileTimes, List<double> readerTimes, List<double> totalTimes, long gcDelta)
	{
		...
		appendResult(builder, "Binary Reader", readerTimes);
		appendResult(builder, "Total", totalTimes);
