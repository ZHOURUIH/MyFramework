...
// Spine 4.1二进制文件扫描器,用于在不修改Spine官方库且不创建SkeletonData、Animation、Timeline等运行时对象的情况下解析并跳过.skel.bytes中的数据,记录生成基础Skeleton文件和独立动作包所需的信息。
public class Spine41BinaryScanner
{
    ...
    public SpineBinaryScanResult scan(byte[] bytes)
    {
        ...
        {
            Spine41Input input = new Spine41Input(stream);
    ...
    protected void scanSkeleton(Spine41Input input, SpineBinaryScanResult result)
    {
        ...
        if (!result.mVersion.StartsWith("4.1", StringComparison.Ordinal))
        {
            throw createFormatException(input, "当前扫描器只支持Spine 4.1，文件版本:" + result.mVersion);
        }
        input.skipFloat(4);
        ...
        result.mSlotCount = skipSlots(input);
    ...
    protected void skipBones(Spine41Input input, bool nonessential)
    {
    ...
    protected int skipSlots(Spine41Input input)
    {
    ...
    protected void skipIkConstraints(Spine41Input input)
    {
    ...
    protected void skipTransformConstraints(Spine41Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipPathConstraints(Spine41Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipSkin(Spine41Input input, bool defaultSkin, bool nonessential)
    {
        int slotCount;
        if (defaultSkin)
        {
    ...
    protected void skipIndexArray(Spine41Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipAttachment(Spine41Input input, bool nonessential)
    {
        input.skipStringRef();
        int attachmentType = input.readByte();
    ...
    protected void skipSequence(Spine41Input input)
    {
    ...
    protected void skipVertices(Spine41Input input, int vertexCount)
    {
        bool weighted = input.readBoolean();
        if (!weighted)
        {
            input.skipFloat(vertexCount << 1);
            return;
    ...
    protected void skipShortArray(Spine41Input input)
    {
        int count = input.readVarInt(true);
        input.skipByte(count * 2L);
    }
    protected bool[] skipEvents(Spine41Input input)
    {
    ...
    protected void skipAnimation(Spine41Input input, int slotCount, bool[] eventHasAudio)
    {
        input.skipVarInt();
    ...
    protected void skipSlotTimelines(Spine41Input input)
    {
    ...
    protected void skipByteValueCurveTimeline(Spine41Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipBoneTimelines(Spine41Input input)
    {
    ...
    protected void skipFloatValueCurveTimeline(Spine41Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipIkConstraintTimelines(Spine41Input input)
    {
        int timelineCount = input.readVarInt(true);
    ...
    protected void skipTransformConstraintTimelines(Spine41Input input)
    {
        int timelineCount = input.readVarInt(true);
        for (int i = 0; i < timelineCount; ++i)
        {
    ...
    protected void skipPathConstraintTimelines(Spine41Input input)
    {
    ...
    protected void skipAttachmentTimelines(Spine41Input input)
    {
    ...
    protected void skipDrawOrderTimeline(Spine41Input input, int slotCount)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipEventTimeline(Spine41Input input, bool[] eventHasAudio)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipCurve(Spine41Input input, int valueCount)
    {
    ...
    protected Exception createFormatException(Spine41Input input, string message)
    {
        return new InvalidDataException(message + "，当前位置:" + input.mPosition + "/" + input.mLength);
    }
    protected class Spine41Input
    {
        ...
        public Spine41Input(Stream input)
        {
