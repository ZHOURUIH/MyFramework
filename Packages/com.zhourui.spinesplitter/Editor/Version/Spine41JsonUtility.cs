...
// Spine 4.1 JSON源数据解析工具。仅Editor使用，不进入运行时。
public static class Spine41JsonUtility
{
    ...
    public static SourceData parse(byte[] bytes)
    {
        ...
        {
            throw new ArgumentException("Spine 4.1 JSON数据为空", nameof(bytes));
        }
        ...
        if (skeleton == null)
        {
            throw new Exception("Spine 4.1 JSON缺少skeleton节点");
        }
        string version = skeleton.Value<string>("spine");
        if (string.IsNullOrEmpty(version) || !version.StartsWith("4.1", StringComparison.Ordinal))
        {
            throw new Exception("当前JSON不是Spine 4.1资源,实际版本:" + version);
        }
    ...
    public static byte[] getAnimationPayloadBytes(SourceData source, string animationName)
    {
        ...
        if (property == null)
        {
            throw new Exception("Spine 4.1 JSON中没有找到动画:" + animationName);
        }
