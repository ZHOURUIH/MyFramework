...
// Spine 4.3二进制文件扫描器,用于在不修改Spine官方库且不创建SkeletonData、Animation、Timeline等运行时对象的情况下解析并跳过.skel.bytes中的数据,记录生成基础Skeleton文件和独立动作包所需的信息。
public class Spine43BinaryScanner
{
    ...
    protected const int CONSTRAINT_IK = 0;
    protected const int CONSTRAINT_PATH = 1;
    protected const int CONSTRAINT_TRANSFORM = 2;
    protected const int CONSTRAINT_PHYSICS = 3;
    protected const int CONSTRAINT_SLIDER = 4;
    protected const int SLIDER_TIME = 0;
    protected const int SLIDER_MIX = 1;
    ...
    public SpineBinaryScanResult scan(byte[] bytes)
    {
        ...
        {
            Spine43Input input = new Spine43Input(stream);
    ...
    protected void scanSkeleton(Spine43Input input, SpineBinaryScanResult result)
    {
        ...
        if (!result.mVersion.StartsWith("4.3", StringComparison.Ordinal))
        {
            throw createFormatException(input, "当前扫描器只支持Spine 4.3，文件版本:" + result.mVersion);
        }
        input.skipFloat(5);
        ...
        int sliderConstraintCount = skipConstraints(input, nonessential);
        ...
        for (int i = 0; i < animationCount; ++i)
        {
            ...
            skipAnimation(input, result.mSlotCount, eventHasAudio, nonessential);
        ...
        result.mRequiredAnimationIndices = new int[sliderConstraintCount];
        for (int i = 0; i < sliderConstraintCount; ++i)
        {
            int animationIndex = input.readVarInt(true);
            if (animationIndex < 0 || animationIndex >= animationCount)
            {
                throw createFormatException(input, "Slider引用的动画索引越界:" + animationIndex + "/" + animationCount);
            }
            result.mRequiredAnimationIndices[i] = animationIndex;
        }
    }
    protected void skipBones(Spine43Input input, bool nonessential)
    {
    ...
    protected int skipSlots(Spine43Input input, bool nonessential)
    {
    ...
    protected int skipConstraints(Spine43Input input, bool nonessential)
    {
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            switch (type)
            {
                case CONSTRAINT_IK:
                {
                    ...
                    if ((flags & 2) != 0) input.skipByte();
                    if ((flags & 32) != 0 && (flags & 64) != 0) input.skipFloat(1);
                ...
                case CONSTRAINT_TRANSFORM:
                {
                    ...
                    input.skipVarInt();
                    int flags = input.readByte();
                    int fromCount = flags >> 5;
                    for (int from = 0; from < fromCount; ++from)
                    {
                        input.skipByte();
                        input.skipFloat(1);
                        int toCount = input.readByte();
                ...
                case CONSTRAINT_SLIDER:
                {
                    ...
                    int flags = input.readByte();
                    if ((flags & 8) != 0) input.skipFloat(1);
                    if ((flags & 16) != 0 && (flags & 32) != 0) input.skipFloat(1);
                    if ((flags & 64) != 0)
                    {
                        input.skipVarInt();
                        input.skipFloat(1);
                ...
                default:
                    throw createFormatException(input, "未知Constraint类型:" + type);
            }
        }
        return sliderCount;
    }
    ...
    protected void skipSkin(Spine43Input input, bool defaultSkin, bool nonessential)
    {
        int slotCount;
        if (defaultSkin)
        {
    ...
    protected void skipIndexArray(Spine43Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipAttachment(Spine43Input input, bool nonessential)
    {
        int flags = input.readByte();
        int attachmentType = flags & 7;
        if ((flags & 8) != 0) input.skipStringRef();
        switch (attachmentType)
        {
            ...
            case ATTACHMENT_MESH:
            {
                ...
                if (triangleCount < 0) throw createFormatException(input, "Mesh三角形数量非法:" + triangleCount);
    ...
    protected void skipSequence(Spine43Input input)
    {
        input.skipVarInt();
        input.skipVarInt();
        input.skipVarInt();
    ...
    protected int skipVertices(Spine43Input input, bool weighted)
    {
        ...
        int boneArrayLength = input.readVarInt(true);
        int position = 0;
        while (position < boneArrayLength)
        {
            int boneCount = input.readVarInt(true);
            ++position;
            if (boneCount < 0 || position + boneCount > boneArrayLength)
            {
                throw createFormatException(input, "加权顶点Bone数组长度非法");
            }
            for (int i = 0; i < boneCount; ++i)
            {
                input.skipVarInt();
    ...
    protected void skipVarIntArray(Spine43Input input, int count)
    {
        if (count < 0)
        {
    ...
    protected bool[] skipEvents(Spine43Input input)
    {
    ...
    protected void skipAnimation(Spine43Input input, int slotCount, bool[] eventHasAudio, bool nonessential)
    {
        input.skipVarInt();
        ...
        skipSliderTimelines(input);
        skipAttachmentTimelines(input);
        skipDrawOrderTimeline(input, slotCount);
        skipDrawOrderFolderTimelines(input);
        skipEventTimeline(input, eventHasAudio);
        if (nonessential) input.skipInt32();
    }
    protected void skipSlotTimelines(Spine43Input input)
    {
    ...
    protected void skipByteValueCurveTimeline(Spine43Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipBoneTimelines(Spine43Input input)
    {
    ...
    protected void skipFloatValueCurveTimeline(Spine43Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipIkConstraintTimelines(Spine43Input input)
    {
        int timelineCount = input.readVarInt(true);
    ...
    protected void skipTransformConstraintTimelines(Spine43Input input)
    {
        int timelineCount = input.readVarInt(true);
        for (int i = 0; i < timelineCount; ++i)
        {
    ...
    protected void skipPathConstraintTimelines(Spine43Input input)
    {
    ...
    protected void skipPhysicsTimelines(Spine43Input input)
    {
    ...
    protected void skipSliderTimelines(Spine43Input input)
    {
        int sliderGroupCount = input.readVarInt(true);
        for (int i = 0; i < sliderGroupCount; ++i)
        {
            input.skipVarInt();
            int timelineCount = input.readVarInt(true);
            for (int j = 0; j < timelineCount; ++j)
            {
                ...
                int frameCount = input.readVarInt(true);
                input.skipVarInt();
                if (timelineType != SLIDER_TIME && timelineType != SLIDER_MIX)
                {
                    throw createFormatException(input, "未知Slider Timeline类型:" + timelineType);
                }
                skipFloatValueCurveTimeline(input, frameCount, 1);
            }
        }
    }
    protected void skipAttachmentTimelines(Spine43Input input)
    {
    ...
    protected void skipDrawOrderTimeline(Spine43Input input, int slotCount)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipDrawOrderFolderTimelines(Spine43Input input)
    {
        int folderCount = input.readVarInt(true);
        for (int i = 0; i < folderCount; ++i)
        {
            int folderSlotCount = input.readVarInt(true);
            skipVarIntArray(input, folderSlotCount);
            int keyCount = input.readVarInt(true);
            for (int key = 0; key < keyCount; ++key)
            {
                input.skipFloat(1);
                int changeCount = input.readVarInt(true);
                if (changeCount > folderSlotCount)
                {
                    throw createFormatException(input, "DrawOrderFolder changeCount超过Folder Slot数量:" + changeCount + " > " + folderSlotCount);
                }
    ...
    protected void skipEventTimeline(Spine43Input input, bool[] eventHasAudio)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipCurve(Spine43Input input, int valueCount)
    {
    ...
    protected Exception createFormatException(Spine43Input input, string message)
    {
        return new InvalidDataException(message + "，当前位置:" + input.mPosition + "/" + input.mLength);
    }
    protected class Spine43Input
    {
        ...
        public Spine43Input(Stream input)
        {
