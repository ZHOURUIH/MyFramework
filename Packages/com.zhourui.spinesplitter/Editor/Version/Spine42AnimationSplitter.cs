...
public static class Spine42AnimationSplitter
{
    ...
    private static SpineAnimationSplitResult splitBinary(string sourceSkeletonAssetPath, bool verifyAfterGenerate, bool showProgress)
    {
        ...
        try
        {
            ...
            Spine42BinaryScanner scanner = new Spine42BinaryScanner();
            ...
            if (string.IsNullOrEmpty(scanResult.mVersion) || !scanResult.mVersion.StartsWith("4.2", StringComparison.Ordinal))
            {
                throw new Exception("当前自动拆分只支持Spine 4.2,实际版本:" + scanResult.mVersion + ",文件:" + result.mSourceSkeletonAssetPath);
            }
            string sourceDirectory = normalizeAssetPath(Path.GetDirectoryName(result.mSourceSkeletonAssetPath));
            ...
            for (int i = 0; i < scanResult.mAnimations.Count; ++i)
            {
                ...
                Spine42SingleAnimationData animationData = createSingleAnimationData(range, sourceBytes, scanResult);
                Spine42AnimationFile.writeAnimation(absoluteFilePath, animationData);
                if (verifyAfterGenerate)
                {
    ...
    private static Spine42AnimationCommonData createAnimationCommonData(SpineBinaryScanResult scanResult, string sourceSkeletonName)
    {
        Spine42AnimationCommonData commonData = new Spine42AnimationCommonData();
        commonData.mFileVersion = Spine42AnimationFile.CURRENT_VERSION;
    ...
    private static Spine42SingleAnimationData createSingleAnimationData(SpineAnimationBinaryRange range, byte[] sourceBytes, SpineBinaryScanResult scanResult)
    {
    ...
    private static void verifyGeneratedCommonFile(string absolutePath, Spine42AnimationCommonData expectedData)
    {
        Spine42AnimationCommonData actualData = Spine42AnimationFile.readCommon(File.ReadAllBytes(absolutePath));
