...
// Spine 4.0二进制文件扫描器,用于在不修改Spine官方库且不创建SkeletonData、Animation、Timeline等运行时对象的情况下解析并跳过.skel.bytes中的数据,记录生成基础Skeleton文件和独立动作包所需的信息。
public class Spine40BinaryScanner
{
    ...
    public SpineBinaryScanResult scan(byte[] bytes)
    {
        ...
        {
            Spine40Input input = new Spine40Input(stream);
    ...
    protected void scanSkeleton(Spine40Input input, SpineBinaryScanResult result)
    {
        ...
        if (!result.mVersion.StartsWith("4.0", StringComparison.Ordinal))
        {
            throw createFormatException(input, "当前扫描器只支持Spine 4.0，文件版本:" + result.mVersion);
        }
        input.skipFloat(4);
    ...
    protected void skipBones(Spine40Input input, bool nonessential)
    {
    ...
    protected int skipSlots(Spine40Input input)
    {
    ...
    protected void skipIkConstraints(Spine40Input input)
    {
    ...
    protected void skipTransformConstraints(Spine40Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipPathConstraints(Spine40Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipSkin(Spine40Input input, bool defaultSkin, bool nonessential)
    {
        int slotCount;
        if (defaultSkin)
        {
    ...
    protected void skipIndexArray(Spine40Input input)
    {
        int count = input.readVarInt(true);
        for (int i = 0; i < count; ++i)
        {
    ...
    protected void skipAttachment(Spine40Input input, bool nonessential)
    {
        input.skipStringRef();
    ...
    protected void skipVertices(Spine40Input input, int vertexCount)
    {
    ...
    protected void skipShortArray(Spine40Input input)
    {
        int count = input.readVarInt(true);
        input.skipByte(count * 2L);
    }
    protected bool[] skipEvents(Spine40Input input)
    {
    ...
    protected void skipAnimation(Spine40Input input, int slotCount, bool[] eventHasAudio)
    {
        input.skipVarInt();
        ...
        skipDeformTimelines(input);
        skipDrawOrderTimeline(input, slotCount);
        skipEventTimeline(input, eventHasAudio);
    }
    protected void skipSlotTimelines(Spine40Input input)
    {
    ...
    protected void skipByteValueCurveTimeline(Spine40Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipBoneTimelines(Spine40Input input)
    {
    ...
    protected void skipFloatValueCurveTimeline(Spine40Input input, int frameCount, int valueCount)
    {
        if (frameCount <= 0)
        {
            return;
    ...
    protected void skipIkConstraintTimelines(Spine40Input input)
    {
        int timelineCount = input.readVarInt(true);
    ...
    protected void skipTransformConstraintTimelines(Spine40Input input)
    {
        int timelineCount = input.readVarInt(true);
        for (int i = 0; i < timelineCount; ++i)
        {
    ...
    protected void skipPathConstraintTimelines(Spine40Input input)
    {
    ...
    protected void skipDeformTimelines(Spine40Input input)
    {
    ...
    protected void skipDrawOrderTimeline(Spine40Input input, int slotCount)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipEventTimeline(Spine40Input input, bool[] eventHasAudio)
    {
        int frameCount = input.readVarInt(true);
        for (int frame = 0; frame < frameCount; ++frame)
        {
    ...
    protected void skipCurve(Spine40Input input, int valueCount)
    {
    ...
    protected Exception createFormatException(Spine40Input input, string message)
    {
        return new InvalidDataException(message + "，当前位置:" + input.mPosition + "/" + input.mLength);
    }
    protected class Spine40Input
    {
        ...
        public Spine40Input(Stream input)
        {
