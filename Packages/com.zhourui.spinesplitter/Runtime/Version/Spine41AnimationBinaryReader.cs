...
// 读取Spine 4.1拆分单动画。Binary payload走Direct Reader，JSON payload复用官方4.1 SkeletonJson动画解析逻辑。
public class Spine41AnimationBinaryReader
{
    private static readonly byte[] ANIMATION_JSON_MAGIC = new byte[] { 0x53, 0x50, 0x4A, 0x41, 0x4E, 0x49, 0x34, 0x31 }; // SPJANI41
    ...
    public Animation readAnimation(byte[] binaryData, int binaryOffset, int binaryLength, string[] strings, SkeletonData skeletonData, float scale, string expectedAnimationName)
    {
        ...
        mScale = scale;
        Spine41AnimationBinaryInput input = new Spine41AnimationBinaryInput(binaryData, binaryOffset, binaryLength, strings);
    ...
    private static Animation readJsonAnimation(byte[] data, int offset, int length, SkeletonData skeletonData, float scale, string expectedAnimationName)
    {
        ...
        {
            throw new MissingMethodException("Spine 4.1 SkeletonJson.ReadAnimation不存在,请确认Spine Runtime版本与资源版本一致");
        }
        offset += ANIMATION_JSON_MAGIC.Length;
        length -= ANIMATION_JSON_MAGIC.Length;
        if (length <= 0)
        {
            throw new InvalidDataException("Spine 4.1单动画JSON为空:" + expectedAnimationName);
        }
        ...
        {
            throw new InvalidDataException("Spine 4.1单动画JSON格式无效:" + expectedAnimationName);
        }
        ...
        {
            throw new InvalidDataException("Spine 4.1单动画JSON解析失败:" + expectedAnimationName, exception.InnerException ?? exception);
        }
        if (skeletonData.Animations.Count != oldAnimationCount + 1)
        {
            throw new InvalidDataException("Spine 4.1单动画JSON解析后Animation数量异常:" + expectedAnimationName);
        }
        ...
        {
            throw new InvalidDataException("Spine 4.1单动画JSON名称不一致:" + expectedAnimationName);
        }
        return animation;
    }
    private Animation readAnimationBody(string name, Spine41AnimationBinaryInput input, SkeletonData skeletonData)
    {
    ...
    // 读取曲线时间轴。
    private Timeline readTimeline(Spine41AnimationBinaryInput input, CurveTimeline1 timeline, float scale)
    {
        float time = input.readFloat();
    ...
    // 读取曲线时间轴。
    private Timeline readTimeline(Spine41AnimationBinaryInput input, CurveTimeline2 timeline, float scale)
    {
        float time = input.readFloat();
    ...
    // 读取曲线时间轴。
    void setBezier(Spine41AnimationBinaryInput input, CurveTimeline timeline, int bezier, int frame, int value, float time1, float time2, float value1, float value2, float scale)
    {
    ...
    private class Spine41AnimationBinaryInput
    {
        ...
        public Spine41AnimationBinaryInput(byte[] data, int offset, int length, string[] strings)
        {
