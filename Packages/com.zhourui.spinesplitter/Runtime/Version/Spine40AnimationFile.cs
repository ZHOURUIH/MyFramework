...
// Spine 4.0单动画公共数据,每个Skeleton只生成一份,保存源Spine版本、源Skeleton Hash和所有动画共同使用的字符串表。
public class Spine40AnimationCommonData
{
...
// Spine 4.0单动画数据,每个文件只保存一个动画名称以及从原始.skel.bytes中复制出的完整动画二进制块。
public class Spine40SingleAnimationData
{
    public int mFileVersion;
    public string mSpineVersion;
    public long mSkeletonHash;
...
// Spine 4.0公共文件和单动画文件读写器,公共字符串表只保存一次,每个.spineanim.bytes只保存一个动画。
public static class Spine40AnimationFile
{
    ...
    private static readonly byte[] COMMON_MAGIC = new byte[] { 0x53, 0x50, 0x4E, 0x43, 0x4F, 0x4D, 0x34, 0x30 };
    private static readonly byte[] ANIMATION_MAGIC = new byte[] { 0x53, 0x50, 0x4E, 0x41, 0x4E, 0x49, 0x34, 0x30 };
    ...
    public static void writeCommon(string filePath, Spine40AnimationCommonData data)
    {
    ...
    public static byte[] writeCommonToBytes(Spine40AnimationCommonData data)
    {
    ...
    public static void writeCommon(Stream stream, Spine40AnimationCommonData data)
    {
    ...
    public static Spine40AnimationCommonData readCommon(byte[] bytes)
    {
        if (bytes == null)
        {
    ...
    public static Spine40AnimationCommonData readCommon(Stream stream)
    {
        validateReadStream(stream);
        using (BinaryReader reader = new BinaryReader(stream, Encoding.UTF8, true))
        {
            verifyMagic(reader, COMMON_MAGIC, "不是有效的Spine 4.0动画公共文件");
            Spine40AnimationCommonData data = new Spine40AnimationCommonData();
    ...
    public static void writeAnimation(string filePath, Spine40SingleAnimationData data)
    {
        if (string.IsNullOrEmpty(filePath))
        {
    ...
    public static byte[] writeAnimationToBytes(Spine40SingleAnimationData data)
    {
        using (MemoryStream stream = new MemoryStream())
        {
            writeAnimation(stream, data);
    ...
    public static void writeAnimation(Stream stream, Spine40SingleAnimationData data)
    {
        ...
        using (BinaryWriter writer = new BinaryWriter(stream, Encoding.UTF8, true))
        {
            ...
            writer.Write(data.mBinaryData.Length);
            writer.Write(data.mBinaryData);
            writer.Flush();
        }
    }
    public static Spine40SingleAnimationData readAnimation(byte[] bytes)
    {
        if (bytes == null)
        {
            throw new ArgumentNullException(nameof(bytes));
    ...
    public static Spine40SingleAnimationData readAnimation(Stream stream)
    {
        validateReadStream(stream);
        using (BinaryReader reader = new BinaryReader(stream, Encoding.UTF8, true))
        {
            verifyMagic(reader, ANIMATION_MAGIC, "不是有效的Spine 4.0单动画文件");
            Spine40SingleAnimationData data = new Spine40SingleAnimationData();
    ...
    private static void validateCommonData(Spine40AnimationCommonData data)
    {
        if (data == null)
        {
    ...
    private static void validateAnimationData(Spine40SingleAnimationData data)
    {
        if (data == null)
        {
            throw new ArgumentNullException(nameof(data));
        ...
        if (data.mBinaryData == null || data.mBinaryData.Length == 0)
        {
            throw new InvalidDataException("动画二进制为空:" + data.mAnimationName);
        }
        if (data.mBinaryData.Length > MAX_ANIMATION_BYTE_COUNT)
        {
            throw new InvalidDataException("动画二进制过大:" + data.mAnimationName + "," + data.mBinaryData.Length);
        }
    }
    private static void validateSpineVersion(string spineVersion, string fileType)
    {
        if (string.IsNullOrEmpty(spineVersion) || !spineVersion.StartsWith("4.0", StringComparison.Ordinal))
        {
