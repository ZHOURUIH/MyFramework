...
public class Spine43AnimationBinaryReader
{
    ...
    private const int SLIDER_TIME = 0;
    private const int SLIDER_MIX = 1;
    ...
    public Animation readAnimation(byte[] binaryData, int binaryOffset, int binaryLength, string[] strings, SkeletonData skeletonData, float scale, string expectedAnimationName)
    {
        ...
        int remaining = input.Length - input.Position;
        if (remaining == 4)
        {
            input.readInt32();
            remaining = 0;
    ...
    private Animation readAnimationBody(string name, Spine43AnimationBinaryInput input, SkeletonData skeletonData)
    {
        ...
        ExposedList<int> animatedBones = new ExposedList<int>(boneCount);
        for (int i = 0; i < boneCount; i++)
        {
            int boneIndex = input.readInt(true);
            animatedBones.Add(boneIndex);
            for (int ii = 0, nn = input.readInt(true); ii < nn; ii++)
            {
        ...
        {
            int index = input.readInt(true);
            PathConstraintData data = (PathConstraintData)skeletonData.Constraints.Items[index];
            for (int ii = 0, nn = input.readInt(true); ii < nn; ii++)
            {
                switch (input.readByte())
                {
        ...
        for (int i = 0, n = input.readInt(true); i < n; i++)
        {
            ...
            for (int ii = 0, nn = input.readInt(true); ii < nn; ii++)
            {
                int type = input.readSByte();
                int frameCount = input.readInt(true);
                int bezierCount = input.readInt(true);
                switch (type)
                {
                    case SLIDER_TIME:
                        timelines.Add(readTimeline(input, new SliderTimeline(frameCount, bezierCount, index), 1));
                        break;
                    case SLIDER_MIX:
                        timelines.Add(readTimeline(input, new SliderMixTimeline(frameCount, bezierCount, index), 1));
                        break;
                    default:
                        throw new SerializationException("Unknown slider timeline type: " + type);
                }
            }
        }
        ...
        for (int i = 0; i < folderCount; ++i)
        {
            ...
            int[] folderSlots = new int[folderSlotCount];
            for (int ii = 0; ii < folderSlotCount; ++ii)
            {
                folderSlots[ii] = input.readInt(true);
            }
            int keyCount = input.readInt(true);
            DrawOrderFolderTimeline timeline = new DrawOrderFolderTimeline(keyCount, folderSlots, skeletonData.Slots.Count);
            for (int key = 0; key < keyCount; ++key)
            {
                timeline.SetFrame(key, input.readFloat(), readDrawOrder(input, folderSlotCount));
            }
            timelines.Add(timeline);
        }
        // Event timeline.
        ...
        if (eventCount > 0)
        {
            ...
            for (int i = 0; i < eventCount; i++)
            {
                ...
                if (e.String == null)
                {
                    e.String = eventData.SetupPose.String;
                }
        ...
        animation.SetTimelines(timelines, animatedBones);
        animation.Duration = duration;
        return animation;
    }
    private int[] readDrawOrder(Spine43AnimationBinaryInput input, int slotCount)
    {
        ...
        int[] unchanged = new int[slotCount - changeCount];
        int originalIndex = 0;
        int unchangedIndex = 0;
    ...
    // 读取曲线时间轴。
    private Timeline readTimeline(Spine43AnimationBinaryInput input, CurveTimeline1 timeline, float scale)
    {
        float time = input.readFloat();
    ...
    // 读取曲线时间轴。
    private Timeline readTimeline(Spine43AnimationBinaryInput input, BoneTimeline2 timeline, float scale)
    {
        float time = input.readFloat();
    ...
    // 读取曲线时间轴。
    void setBezier(Spine43AnimationBinaryInput input, CurveTimeline timeline, int bezier, int frame, int value, float time1, float time2, float value1, float value2, float scale)
    {
    ...
    private class Spine43AnimationBinaryInput
    {
        ...
        public Spine43AnimationBinaryInput(byte[] data, int offset, int length, string[] strings)
        {
