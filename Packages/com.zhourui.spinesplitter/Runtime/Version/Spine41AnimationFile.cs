...
// Spine 4.1单动画公共数据,每个Skeleton只生成一份,保存源Spine版本、源Skeleton Hash和所有动画共同使用的字符串表。
public class Spine41AnimationCommonData
{
...
// Spine 4.1单动画数据,每个文件只保存一个动画名称以及从原始.skel.bytes中复制出的完整动画二进制块。
public class Spine41SingleAnimationData
{
    public int mFileVersion;
    public string mSpineVersion;
    public long mSkeletonHash;
...
// Spine 4.1公共文件和单动画文件读写器,公共字符串表只保存一次,每个.spineanim.bytes只保存一个动画。
public static class Spine41AnimationFile
{
    ...
    private static readonly byte[] COMMON_MAGIC = new byte[] { 0x53, 0x50, 0x4E, 0x43, 0x4F, 0x4D, 0x34, 0x31 };
    private static readonly byte[] ANIMATION_MAGIC = new byte[] { 0x53, 0x50, 0x4E, 0x41, 0x4E, 0x49, 0x34, 0x31 };
    ...
    public static void writeCommon(string filePath, Spine41AnimationCommonData data)
    {
    ...
    public static byte[] writeCommonToBytes(Spine41AnimationCommonData data)
    {
    ...
    public static void writeCommon(Stream stream, Spine41AnimationCommonData data)
    {
    ...
    public static Spine41AnimationCommonData readCommon(byte[] bytes)
    {
        if (bytes == null)
        {
    ...
    public static Spine41AnimationCommonData readCommon(Stream stream)
    {
        validateReadStream(stream);
        using (BinaryReader reader = new BinaryReader(stream, Encoding.UTF8, true))
        {
            verifyMagic(reader, COMMON_MAGIC, "不是有效的Spine 4.1动画公共文件");
            Spine41AnimationCommonData data = new Spine41AnimationCommonData();
    ...
    public static void writeAnimation(string filePath, Spine41SingleAnimationData data)
    {
        if (string.IsNullOrEmpty(filePath))
        {
    ...
    public static byte[] writeAnimationToBytes(Spine41SingleAnimationData data)
    {
        using (MemoryStream stream = new MemoryStream())
        {
            writeAnimation(stream, data);
    ...
    public static void writeAnimation(Stream stream, Spine41SingleAnimationData data)
    {
    ...
    public static Spine41SingleAnimationData readAnimation(byte[] bytes)
    {
        if (bytes == null)
        {
            throw new ArgumentNullException(nameof(bytes));
    ...
    public static Spine41SingleAnimationData readAnimationNoCopy(byte[] bytes)
    {
        if (bytes == null)
        {
            throw new ArgumentNullException(nameof(bytes));
        ...
        using (BinaryReader reader = new BinaryReader(stream, Encoding.UTF8, true))
        {
            verifyMagic(reader, ANIMATION_MAGIC, "不是有效的Spine 4.1单动画文件");
            Spine41SingleAnimationData data = new Spine41SingleAnimationData();
    ...
    public static Spine41SingleAnimationData readAnimation(Stream stream)
    {
        validateReadStream(stream);
        using (BinaryReader reader = new BinaryReader(stream, Encoding.UTF8, true))
        {
    ...
    private static void validateCommonData(Spine41AnimationCommonData data)
    {
        if (data == null)
        {
    ...
    private static void validateAnimationData(Spine41SingleAnimationData data)
    {
        if (data == null)
        {
            throw new ArgumentNullException(nameof(data));
    ...
    private static void getBinaryData(Spine41SingleAnimationData data, out byte[] binaryData, out int binaryOffset, out int binaryLength)
    {
    ...
    {
        if (string.IsNullOrEmpty(spineVersion) || !spineVersion.StartsWith("4.1", StringComparison.Ordinal))
        {
