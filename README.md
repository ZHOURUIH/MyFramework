...
```text
复制 ProjectTemplate~ 中的默认项目文件
生成 Assets/Resources/Scene/start.unity
生成 Assets/Resources/Settings/FrameSettings.asset
生成 ProjectSettings/MyFrameworkSettings.asset
将 GameEntry 脚本挂接到 start.unity 的 GameEntry 节点
将 start.unity 添加到 Build Settings
```

如果选择 HybridCLR + Obfuz 初始化，还会自动处理：

```text
安装 HybridCLR
安装 Obfuz
添加 USE_HYBRID_CLR / USE_OBFUZ 宏
设置生成代码输出目录到 Assets/Scripts/AutoGenerated
```
...
框架和默认启动场景仍然会使用部分旧输入接口，例如 `UnityEngine.Input` 和 `StandaloneInputModule`。

如果工程启用了 Input System Package，并且运行时报：
...
MyFramework 现在以 UPM Package 形式组织，框架代码位于：
...
| 目录           | 说明              |
| ------------ | --------------- |
| Runtime/Frame_Base   | 不可热更，最基础的框架代码   |
| Runtime/Frame_Game   | 不可热更，项目非热更代码的框架 |
| Runtime/Frame_HotFix | 可热更，框架层热更代码     |
| Editor/Editor_Frame  | 框架编辑器工具与菜单       |
| Editor/EditorRes_Frame | 框架编辑器资源与辅助逻辑    |
| ProjectTemplate~     | 初始化工程时复制到用户工程的模板文件 |

安装插件后，用户工程中由初始化菜单生成项目入口和默认文件：
...
| 目录           | 说明              |
| ------------ | --------------- |
| Assets/Scripts/Game   | 不可热更，项目应用层启动逻辑 |
| Assets/Scripts/HotFix | 可热更，项目应用层热更代码 |
| Assets/Scripts/AutoGenerated | HybridCLR / Obfuz 等生成代码 |
| Assets/Resources/Scene | 默认启动场景 |
| Assets/Resources/Settings | 运行时框架配置 |
...
为了更直观地展示 MyFramework 在实际 Unity 项目中的使用方式，现已提供一个基于本框架开发的完整塔防游戏 Demo：

**项目地址：**

https://github.com/ZHOURUIH/MyFramework_TDDemo

MyFramework_TDDemo 不是单个功能的代码片段，而是一个按照 MyFramework 项目结构组织的完整 Unity 工程，可以用于了解框架从项目启动、场景切换到具体游戏业务的整体落地方式。
...
* `Game / HotFix / AutoGenerated` 项目代码分层
* HybridCLR 热更新工程结构
* Obfuz 代码混淆接入
* SceneProcedure 场景流程管理
* UI 界面组织与代码自动生成
* CSV 配置表及代码生成
* 资源同步与异步加载
* 炮塔、怪物、子弹等游戏对象的生命周期管理
* 命令系统、事件系统和状态管理
* 音效、特效与多语言模块的项目组织方式
* MyFramework 各个系统与实际游戏业务之间的协作方式
...
使用 Unity 打开工程，等待 Package Manager 完成依赖加载后，打开启动场景：
...
Demo 已经在 `Packages/manifest.json` 中配置 MyFramework、HybridCLR 和 Obfuz 等依赖，一般不需要再次手动导入框架。

> Demo 主要用于展示 MyFramework 的项目结构、开发流程和实际使用方式。建议先运行 Demo 了解整体流程，再结合框架文档查看具体模块。
