...
{
	protected List<Vector3> mPointList = new();		// 点列表
	protected MeshRenderer mMeshRenderer;			// 线条的mesh渲染组件
	protected Transform mTransform;					// 线条变换组件
	protected GameObject mObject;					// 线条物体
	protected Mesh mMesh;							// 显示的Mesh
	protected float mWidth = 10;					// 宽度的一半
	public void init(GameObject obj)
	{
		mObject = obj;
		mObject.SetActive(true);
		mObject.TryGetComponent(out mTransform);
		mObject.TryGetComponent(out mMeshRenderer);
		mObject.TryGetComponent<MeshFilter>(out var filter);
		mMesh = filter.mesh;
	}
