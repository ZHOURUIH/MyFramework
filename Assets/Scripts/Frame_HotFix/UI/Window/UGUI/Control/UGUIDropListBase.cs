...
public abstract class UGUIDropListBase : WindowObjectUGUI
{
	...
	protected IDropItem mCurSelect;					// 当前选中的选项
	protected int mSelectIndex;                     // 当前选中的下标
	public UGUIDropListBase(IWindowObjectOwner parent) : base(parent){}
	...
	public void setOptions(List<string> options, List<int> customValue = null, bool triggerEvent = true) 
	{
		...
		createAllItem(options, customValue);
	...
	{
		IDropItem select = getSelectByIndex(value);
		if (select != null)
		{
			mSelectIndex = value;
			mCurSelect = select;
	...
	public void dropItemClick(IDropItem item)
	{
	...
	//------------------------------------------------------------------------------------------------------------------------------
	protected abstract void createAllItem(List<string> options, List<int> customValue);
	protected abstract IDropItem getSelectByIndex(int index);
	protected abstract int getIndexOfItem(IDropItem item);
