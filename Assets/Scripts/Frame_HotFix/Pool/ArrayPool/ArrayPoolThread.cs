...
public class ArrayPoolThread : FrameSystem
{
	...
	{
		Type type = typeof(T);
		if (!mInusedList.TryGetValue(type, out var typeList))
		{
			Debug.LogError("remove from Inused List failed! type : " + type);
			return;
		}
		foreach (T[] array in allArrayList)
		{
			// 从使用列表移除,要确保操作的都是从本类创建的实例
			if (!typeList.TryGetValue(array.Length, out var arrayList) || !arrayList.Remove(array))
			{
				Debug.LogError("remove from Inused List failed! size : " + array.Length);
			}
		}
