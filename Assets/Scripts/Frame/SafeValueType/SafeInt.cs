...
public struct SafeInt : IEquatable<SafeInt>
{
	...
	private static int mValue0 = 135386587;
	private static int mValue1 = 335456345;
	private static int mValue2 = 212346587;
	private static int mValue3 = 311156345;
	private static int mValue5 = 437;
	...
	public int get() 
	{
		...
		if (gikoweg < mValue0 || gikoweg > mValue1 || woghjwe < mValue2 || woghjwe > mValue3 || (woghjwe + gikoweg) % mValue5 != 0)
		{
			...
			logError("校验失败");
#else
			FrameBase.mGameFramework.onMemoryModified(3, wghwe, woghjwe, gikoweg, gikowjeg);
#endif
		}
		int value = 0;
		switch (wghwe)
		...
		if (curValue != (gikowjeg ^ woghjwe))
		{
			...
			logError("校验失败");
#else
			FrameBase.mGameFramework.onMemoryModified(4, wghwe, woghjwe, gikoweg, gikowjeg);
#endif
		}
		return curValue;
	}
	...
	private void generate()
	{
		...
		// 经过调整以后mNoJustThis会变小,所以如果超过最大值需要减少到范围内
		if (gikoweg < mValue0)
		{
			gikoweg += mValue5;
		}
