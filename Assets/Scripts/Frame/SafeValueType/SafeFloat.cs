...
public struct SafeFloat : IEquatable<SafeFloat>
{
	...
	private static int mValue0 = 56859348;
	private static int mValue1 = 91239999;
	private static int mValue3 = 33776454;
	private static int mValue4 = 81239999;
	private static int mValue5 = 737;
	...
	public float get()  
	{
		...
		if (qwfb < mValue0 || qwfb > mValue1 || qgoqjg < mValue3 || qgoqjg > mValue4 || (qgoqjg + qwfb) % mValue5 != 0)
		{
			...
			logError("校验失败");
#else
			FrameBase.mGameFramework.onMemoryModified(5, qgoqjg, qwfb, asgihfasg, khsgh);
#endif
		}
		int value = 0;
		switch (khsgh)
		...
		if (!isFloatEqual(curValue, (asgihfasg ^ qgoqjg) * 0.0001f, 0.01f))
		{
			...
			logError("校验失败");
#else
			FrameBase.mGameFramework.onMemoryModified(6, qgoqjg, qwfb, asgihfasg, khsgh);
#endif
		}
		return curValue;
	}
	...
	private void generate()
	{
		...
		// 经过调整以后mFuckAll会变小,所以如果超过最大值需要减少到范围内
		if (qwfb < mValue0)
		{
			qwfb += mValue5;
		}
