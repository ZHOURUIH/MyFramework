...
// 触摸点信息,暂未实现长按检测
public class TouchPoint : FrameBase
{
	...
	public void pointUp(Vector3 pos, List<DeadClick> deadTouchList)
	{
		...
		mUpTime = DateTime.Now;
		mCurPosition = pos;
		mClick = lengthLess(mDownPosition - mCurPosition, FrameDefine.CLICK_LENGTH);
		// 遍历一段时间内已经完成点击的触点列表,查看有没有点坐标相近,时间相近的触点
		if (mClick)
		{
			int deadCount = deadTouchList.Count;
			for (int i = 0; i < deadCount; ++i)
			{
				DeadClick deadClick = deadTouchList[i];
				if ((mUpTime - deadClick.mClickTime).TotalSeconds < FrameDefine.DOUBLE_CLICK_TIME && 
					lengthLess(mCurPosition - deadClick.mClickPosition, FrameDefine.CLICK_LENGTH))
				{
					mDoubleClick = true;
					break;
				}
