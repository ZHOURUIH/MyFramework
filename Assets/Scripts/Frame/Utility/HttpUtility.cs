...
public class HttpUtility : FrameSystem
{
	...
	{
		using (new ThreadLockScope(ThreadListLock))
		{
	...
	{
		return post(url, null, -1, "application/x-www-form-urlencoded", header, callback);
	}
	...
	protected static string generateGetParams(Dictionary<string, string> paramList)
	{
		...
		using (new ClassScope<MyStringBuilder>(out var parameters))
		{
			parameters.append("?");
