...
public class myUISpriteAnim : myUISprite, IUIAnimation
{
	...
	public override void setAtlas(UGUIAtlasPtr atlas, bool clearSprite = false)
	{
		if(atlas.mAtlas == getAtlas().mAtlas)
		{
			return;
		}
		// 改变图集时先停止播放
	...
	public void setTextureSet(string textureSetName)
	{
		...
		if (mAtlasPtr.isValid() && !isEmpty(mTextureSetName))
		{
			int index = 0;
			while(true)
			{
				string name = mTextureSetName + "_" + IToS(index++);
				Sprite sprite = mAtlasPtr.getSprite(name);
				if (sprite == null)
				{
					break;
				}
	...
	{
		if (clear && mPlayEndCallbackList != null && mPlayEndCallbackList.Count > 0)
		{
			using (new ListScope<TextureAnimCallback>(out var tempList))
			{
	...
	protected void onPlaying(AnimControl control, int frame, bool isPlaying)
	{
		...
		setSprite(mSpriteList[mControl.getCurFrameIndex()]);
		// 使用位置列表进行校正
	...
	protected void onPlayEnd(AnimControl control, bool callback, bool isBreak)
	{
		...
		if (mPlayEndCallbackList != null && mPlayEndCallbackList.Count > 0)
		{
			if (callback)
			{
				using (new ListScope<TextureAnimCallback>(out var tempList))
