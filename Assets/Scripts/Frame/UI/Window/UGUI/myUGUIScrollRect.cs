...
public class myUGUIScrollRect : myUGUIObject
{
	...
	{
		if (mScrollRect.content == null || mScrollRect.viewport == null)
		{
			logError("滑动窗口的content或者viewport未设置,window:" + mName + ", layout:" + mLayout.getName());
		}
		mContent = content;
		mViewport = viewport;
		...
		mScrollRectImage = mObject.GetComponent<Image>();
		mViewportImage = mViewport.tryGetUnityComponent<Image>();		
		if (mScrollRectImage == null || !mScrollRectImage.raycastTarget ||
			mViewportImage == null || !mViewportImage.raycastTarget)
		{
			logError("需要ScrollRect和viewport都有一个的Image组件,并且是开启射线检测, window:" + mName + ", layout:" + mLayout.getName());
		}
	}
	...
	public void autoAdjustContent(CONTENT_ADJUST adjustType, Vector2 itemSize)
	{
		if (mScrollRect.vertical)
		{
			...
			{
				autoGridVertical(mContent, true);
			}
			else if(adjustType == CONTENT_ADJUST.FIXED_WIDTH_OR_HEIGHT)
			{
				autoGridFixedRootWidth(mContent, itemSize, true);
			}
		...
		else if (mScrollRect.horizontal)
		{
			if (adjustType == CONTENT_ADJUST.SINGLE_COLUMN_OR_LINE)
			{
				autoGridHorizontal(mContent, true);
			}
			else if(adjustType == CONTENT_ADJUST.FIXED_WIDTH_OR_HEIGHT)
			{
				autoGridFixedRootHeight(mContent, itemSize, true);
			}
