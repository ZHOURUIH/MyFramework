...
public class myUGUIRawImageAnim : myUGUIRawImage, IUIAnimation
{
	...
	{
		base.destroy();
		int count = mTextureList.Count;
		for (int i = 0; i < count; ++i)
		{
			var temp = mTextureList[i];
	...
	public void setTexturePath(string texturePath, string texturePreName, int textureCount)
	{
		...
		{
			mTextureList.Add(mResourceManager.loadResource<Texture>(preName + IToS(i) + ".png"));
		}
	...
	{
		if (clear && mPlayEndCallback != null && mPlayEndCallback.Count > 0)
		{
	...
	protected void onPlayEnd(AnimControl control, bool callback, bool isBreak)
	{
		...
		if (mPlayEndCallback != null && mPlayEndCallback.Count > 0)
		{
			if (callback)
			{
				using (new ListScope<TextureAnimCallback>(out var tempList))
