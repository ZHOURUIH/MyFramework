...
public class myUGUIObject : myUIObject
{
	...
	{
		return getPositionNoPivot(mRectTransform);
	}
	...
	protected void ensureColliderSize()
	{
		...
		if (!isFloatEqual(mRectTransform.rect.width, mBoxCollider.size.x) || 
			!isFloatEqual(mRectTransform.rect.height, mBoxCollider.size.y))
		{
			mBoxCollider.size = mRectTransform.rect.size;
			mBoxCollider.center = multiVector2(mRectTransform.rect.size, new Vector2(0.5f, 0.5f) - mRectTransform.pivot);
		}
