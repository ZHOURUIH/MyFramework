...
public class myUIObject : Transformable, IMouseEventCollect, IEquatable<myUIObject>
{
	...
	public override void update(float elapsedTime)
	{
		...
		{
			if (mLongPressLengthThreshold < 0.0f || 
	...
	public virtual void onMouseUp(Vector3 mousePos, int touchID)
	{
		...
		if (lengthLess(mMouseDownPosition - mousePos, FrameDefine.CLICK_LENGTH) && 
			(DateTime.Now - mMouseDownTime).TotalSeconds < FrameDefine.CLICK_TIME)
		{
			mObjectPreClickCallback?.Invoke(this, mObjectPreClickUserData);
			mClickCallback?.Invoke(this, mousePos);
			if (mDoubleClickCallback != null && (DateTime.Now - mLastClickTime).TotalSeconds < FrameDefine.DOUBLE_CLICK_TIME)
			{
				mDoubleClickCallback(this, mousePos);
			}
			mLastClickTime = DateTime.Now;
		}
