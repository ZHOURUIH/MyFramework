...
public class LayoutManager : FrameSystem
{
	...
	public LayoutManager()
	{
		...
		mUGUIRoot = LayoutScript.newUIObject<myUGUICanvas>(null, null, getGameObject(UGUI_ROOT, true));
	}
	...
	public override void update(float elapsedTime)
	{
		...
		foreach (var item in mLayoutList.startForeach("LayoutManager:80"))
		{
			try
			{
			...
			{
				logException(e,"界面:" + item.Value.getName());
			}
		}
		mLayoutList.endForeach();
	}
	public override void onDrawGizmos()
	{
		foreach (var item in mLayoutList.startForeach("LayoutManager:101"))
		{
	...
	public override void lateUpdate(float elapsedTime)
	{
		...
		foreach (var item in mLayoutList.startForeach("LayoutManager:110"))
		{
			try
			{
	...
	public override void willDestroy()
	{
		...
		foreach (var item in mLayoutList.startForeach("LayoutManager:126"))
		{
			item.Value.destroy();
		}
		mLayoutList.endForeach();
	...
	public GameLayout createLayout(LayoutInfo info, bool async)
	{
		...
		string fullPath = R_UGUI_PREFAB_PATH + path + ".prefab";
		// 如果是异步加载则,则先加入列表中
		if (async)
		{
	...
	{
		foreach (var item in mLayoutList.startForeach("LayoutManager:320"))
		{
	...
	{
		if (!mLayoutAsyncList.TryGetValue(asset.name, out LayoutInfo info))
		{
			return;
		}
