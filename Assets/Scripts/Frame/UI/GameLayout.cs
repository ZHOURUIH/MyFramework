...
{
	protected Dictionary<int, myUIObject> mGameObjectSearchList;	// 用于根据GameObject查找UI,key是GameObject的InstanceID
	protected SafeDictionary<int, myUIObject> mNeedUpdateList;		// mObjectList中需要更新的窗口列表
	protected SafeDictionary<int, myUIObject> mObjectList;			// 布局中UI物体列表,用于保存所有已获取的UI
	...
	public GameLayout()
	{
		mGameObjectSearchList = new Dictionary<int, myUIObject>();
		mNeedUpdateList = new SafeDictionary<int, myUIObject>();
	...
	public void init()
	{
		...
		setRectSize(rectTransform, new Vector2(STANDARD_WIDTH, STANDARD_HEIGHT));
	...
	public void update(float elapsedTime)
	{
		...
		try
		{
			foreach (var item in mNeedUpdateList.startForeach())
			{
	...
	{
		if (mScript == null || !mScriptInited || visible == mRoot.isActive())
		{
			return;
		}
	...
	{
		mGameObjectSearchList.TryGetValue(go.GetInstanceID(), out myUIObject obj);
		return obj;
	}
	// get
	...
	protected void setUIDepth(myUIObject window, int orderInParent, bool includeSelf = true, bool ignoreInactive = false)
	{
		...
		if (includeSelf && (transform.childCount > 0 || mGlobalTouchSystem.isColliderRegisted(window) || canvas != null))
		{
			...
			else
			{
				...
				window.setDepth(parentDepth, orderInParent);
			}
		}
