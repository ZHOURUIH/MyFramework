...
#if UNITY_EDITOR
		if (includeSelf && window.isActive() && window.getCollider() != null && window.isColliderForClick() && !mGlobalTouchSystem.isColliderRegisted(window))
		{
			logError("窗口拥有碰撞体,但是由于未注册碰撞体,所以无法为窗口计算深度,Layout:" + getName() + ", Window:" + getTransformPath(window.getTransform()));
		}
#endif
		// 先设置当前窗口的深度
		// 只有当拥有子节点或者窗口带BoxCollider时,才会计算窗口的深度
		if (includeSelf && (window.getTransform().childCount > 0 || mGlobalTouchSystem.isColliderRegisted(window)))
		{
