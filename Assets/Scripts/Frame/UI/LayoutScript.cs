...
public abstract class LayoutScript : DelayCmdWatcher
{
	...
	public LayoutScript()
	{
		mNeedUpdate = true;
#if UNITY_EDITOR
		mScrollViewRegisteList = new HashSet<myUGUIScrollRect>();
		mInputFieldRegisteList = new HashSet<IInputField>();
#endif
	}
	...
	public void bindPassOnlyParent(myUIObject obj)
	{
		...
		obj.setDepth(obj.getParent().getDepth(), obj.getDepth().getOrderInParent());
		// 刷新深度
	...
	public virtual void onReset()
	{
		...
		if (!mRegisterChecked)
		{
			...
			var scrollViewList = mRoot.getObject().GetComponentsInChildren<ScrollRect>();
			...
			var inputFieldList = mRoot.getObject().GetComponentsInChildren<InputField>();
	...
	public GameObject instantiate(myUIObject parent, string prefabPath, string name, int tag = 0)
	{
		...
		Profiler.BeginSample("mPrefabPoolManager.createObject");
#endif
		GameObject go = mPrefabPoolManager.createObject(prefabPath, tag, parent.getObject());
#if UNITY_EDITOR || DEVELOPMENT_BUILD
		Profiler.EndSample();
#endif
		if (go != null)
	...
	public void instantiateAsync(string prefabPath, string name, int tag, CreateObjectCallback callback)
	{
		mPrefabPoolManager.createObjectAsync(prefabPath, tag, (GameObject go, object userData) =>
		{
			if (go != null)
			{
				go.name = name;
	...
	public void destroyObject(ref myUGUIObject obj, bool immediately = false)
	{
		destroyObject(obj, immediately);
		obj = null;
	}
