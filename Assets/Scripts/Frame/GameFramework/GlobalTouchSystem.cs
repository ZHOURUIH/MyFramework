...
public class GlobalTouchSystem : FrameComponent
{
	...
	private List<DistanceSortHelper> mTempSortList;
	private List<IMouseEventCollect> mTempEndMultiWindow;
		...
		:base(name)
	{
		...
		mTempSortList = new List<DistanceSortHelper>();
		...
		mTempEndMultiWindow = new List<IMouseEventCollect>();
		...
		if (touchCount >= 2)
		{
			...
			foreach (var item in mTouchInWindow)
			{
				...
				if (touchesInWindow.Count == 2)
				{
					...
					if (isEnd)
					{
						mTempEndMultiWindow.Add(item.Key);
					}
					else if (isStart)
					{
			...
			// 结束多点触控的窗口
			foreach (var item in mTempEndMultiWindow)
			{
	...
	protected void raycastMovableObject(ref Ray ray, List<IMouseEventCollect> moveObjectList, List<IMouseEventCollect> retList, ref bool continueRay, bool clearList = true)
	{
		...
		foreach (var box in moveObjectList)
		{
			...
			{
				mTempSortList.Add(new DistanceSortHelper(getSquaredLength(hit.point - ray.origin), box));
			}
		}
		// 根据相交点由由近到远的顺序排序
		mTempSortList.Sort(DistanceSortHelper.distanceAscend);
		foreach (var item in mTempSortList)
		{
			...
			for (int i = 0; i < count; ++i)
			{
				...
				if (window.isActive() && window.isHandleInput() && window.getCollider().Raycast(ray, out hit, 10000.0f))
				{
					...
					else
					{
						...
						if ((!ignorePassRay && !window.isPassRay()) || (maxCount > 0 && retList.Count >= maxCount))
						{
							continueRay = false;
							break;
						}
