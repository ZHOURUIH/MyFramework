...
public class MathUtility : StringUtility
{
	...
	private static PathNode[] mTempNodeList = null;
	private static float[] sin_tb = null;
	private static float[] cos_tb = null;
	private const int mMaxFFTCount = 1024 * 8;
	private static Complex[] mComplexList = new Complex[mMaxFFTCount];
	private static int[] mPow2ValueList = new int[32];
	...
	public static int randomInt(int min, int max, bool force = false)
	{
		if (min == max)
		{
			return min;
	...
	public static bool canConnectPoint(List<Vector2> vertice, int index0, int index1)
	{
		...
		// 计算该多边形的法线
		Vector3 normal = cross(new Vector3(nextDir.x, 0.0f, nextDir.y), new Vector3(lastDir.x, 0.0f, lastDir.y));
		...
		for (int i = 0; i < verticeCount; ++i)
		{
			...
			if (intersectLineSection(new Line2(vertice[i], vertice[(i + 1) % verticeCount]), new Line2(vertice[index0], vertice[index1]), out intersectPoint, false))
			{
				return false;
			}
		}
	...
	public static bool intersect(Line2 line, Rect rect, bool checkEndPoint = false)
	{
		...
		Vector2 intersect;
		if(intersectLineSection(line, new Line2(leftTop, rightTop), out intersect, checkEndPoint))
		{
			return true;
		}
		if (intersectLineSection(line, new Line2(leftBottom, rightBottom), out intersect, checkEndPoint))
		{
			return true;
		}
		if (intersectLineSection(line, new Line2(leftBottom, leftTop), out intersect, checkEndPoint))
		{
			return true;
		}
		if (intersectLineSection(line, new Line2(rightBottom, rightTop), out intersect, checkEndPoint))
		{
			return true;
		}
		return false;
	...
	{
		return isInRange(value.x, point0.x, point1.x) && isInRange(value.y, point0.y, point1.y);
	}
