...
public class UnityUtility : FileUtility
{
	...
	public static void logError(string info, bool isMainThread = true)
	{
		...
		// 游戏中的错误日志
		FrameBase.mFrameLogSystem?.logGameError(info);
...
#else
		return CommonDefine.F_TEMPORARY_CACHE_PATH;
#endif
	}
	...
	public static Collider2D[] overlapCollider(Collider2D collider, int layer = -1)
	{
		Collider2D[] hitColliders = null;
		if (collider is BoxCollider2D)
		{
			hitColliders = overlapBox(collider as BoxCollider2D, layer);
		}
		else if (collider is CircleCollider2D)
		{
			hitColliders = overlapSphere(collider as CircleCollider2D, layer);
		}
		else if (collider is CapsuleCollider2D)
		{
			hitColliders = overlapCapsule(collider as CapsuleCollider2D, layer);
		}
		return hitColliders;
	}
	public static Collider[] overlapCollider(Collider collider, int layer = -1)
	{
		...
		{
			hitColliders = overlapBox(collider as BoxCollider, layer);
		}
		else if (collider is SphereCollider)
		{
			hitColliders = overlapSphere(collider as SphereCollider, layer);
		}
		else if (collider is CapsuleCollider)
		{
			hitColliders = overlapCapsule(collider as CapsuleCollider, layer);
		}
		return hitColliders;
	}
	public static Collider[] overlapBox(BoxCollider collider, int layer = -1)
	{
	...
	public static Collider2D[] overlapBox(BoxCollider2D collider, int layer = -1)
	{
		Transform transform = collider.transform;
	...
	public static Collider[] overlapSphere(SphereCollider collider, int layer = -1)
	{
		Transform transform = collider.transform;
		Vector3 colliderWorldPos = localToWorld(transform, collider.center);
		return Physics.OverlapSphere(colliderWorldPos, collider.radius, layer);
	}
	public static Collider2D[] overlapSphere(CircleCollider2D collider, int layer = -1)
	{
		Transform transform = collider.transform;
		Vector2 colliderWorldPos = localToWorld(transform, collider.offset);
		return Physics2D.OverlapCircleAll(colliderWorldPos, collider.radius, layer);
	}
	public static Collider[] overlapCapsule(CapsuleCollider collider, int layer = -1)
	{
		Transform transform = collider.transform;
	...
	public static Collider2D[] overlapCapsule(CapsuleCollider2D collider, int layer = -1)
	{
		Transform transform = collider.transform;
