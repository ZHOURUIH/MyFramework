...
public class StringUtility : BinaryUtility
{
	...
	// 使用Length == 0判断是否为空字符串是最快的,但是如果调用此isEmpty函数,则会因为入栈和出栈导致消耗增加
	// 所以只在需要判断的地方使用Length == 0即可
	...
	public static void removeEndString(ref string originStr, string endString)
	{
		if (string.IsNullOrEmpty(endString) || string.IsNullOrEmpty(originStr))
		{
			return;
		}
		if (endWith(originStr, endString))
		{
			originStr = originStr.Substring(0, originStr.Length - endString.Length);
		}
	}
	// 移除整个stream中的最后一个字符key
	...
	{
		rightToLeft(ref str);
		string ret = FrameBase.EMPTY_STRING;
	...
	{
		string str = FrameBase.EMPTY_STRING;
	...
	{
		string newString = FrameBase.EMPTY_STRING;
	...
	public static string checkFloatString(string str, string valid = FrameBase.EMPTY_STRING)
	{
		return checkIntString(str, "." + valid);
	}
	public static string checkIntString(string str, string valid = FrameBase.EMPTY_STRING)
	{
		return checkString(str, "-0123456789" + valid);
	}
	public static string checkUIntString(string str, string valid = FrameBase.EMPTY_STRING)
	{
		return checkString(str, "0123456789" + valid);
	}
	...
	{
		string byteString = FrameBase.EMPTY_STRING;
	...
	{
		string hexString = FrameBase.EMPTY_STRING;
