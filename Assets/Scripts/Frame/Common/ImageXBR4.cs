...
public class ImageXBR4 : FrameBase
{
	...
	public static Texture2D convertImage(Texture2D tex, int scale = 4)
	{
		...
		for(int i = 0; i < height; ++i)
		{
			...
			{
				newPixel[j + i * width] = fragment(pixels, new Vector2((float)j / width, (float)i / height), width, height, scale);
			}
		}
		Texture2D newTex = new Texture2D(tex.width * scale, tex.height * scale, TextureFormat.RGBA32, false);
	...
	protected static Vector4 fragment(Color[] pixels, Vector2 texCoord, int width, int height, int scale)
	{
		...
		Vector2 ps = new Vector2(1.0f / (width / scale), 1.0f / (height / scale));
		float dx = ps.x;
		float dy = ps.y;
		...
		src[21] = tex2D(pixels, width, height, new Vector2(t1.x, t1.w));
		src[22] = tex2D(pixels, width, height, new Vector2(t1.y, t1.w));
		src[23] = tex2D(pixels, width, height, new Vector2(t1.z, t1.w));
		src[6] = tex2D(pixels, width, height, new Vector2(t2.x, t2.w));
		src[7] = tex2D(pixels, width, height, new Vector2(t2.y, t2.w));
		src[8] = tex2D(pixels, width, height, new Vector2(t2.z, t2.w));
		src[5] = tex2D(pixels, width, height, new Vector2(t3.x, t3.w));
		src[0] = tex2D(pixels, width, height, new Vector2(t3.y, t3.w));
		src[1] = tex2D(pixels, width, height, new Vector2(t3.z, t3.w));
		src[4] = tex2D(pixels, width, height, new Vector2(t4.x, t4.w));
		src[3] = tex2D(pixels, width, height, new Vector2(t4.y, t4.w));
		src[2] = tex2D(pixels, width, height, new Vector2(t4.z, t4.w));
		src[15] = tex2D(pixels, width, height, new Vector2(t5.x, t5.w));
		src[14] = tex2D(pixels, width, height, new Vector2(t5.y, t5.w));
		src[13] = tex2D(pixels, width, height, new Vector2(t5.z, t5.w));
		src[19] = tex2D(pixels, width, height, new Vector2(t6.x, t6.y));
		src[18] = tex2D(pixels, width, height, new Vector2(t6.x, t6.z));
		src[17] = tex2D(pixels, width, height, new Vector2(t6.x, t6.w));
		src[9] = tex2D(pixels, width, height, new Vector2(t7.x, t7.y));
		src[10] = tex2D(pixels, width, height, new Vector2(t7.x, t7.z));
		src[11] = tex2D(pixels, width, height, new Vector2(t7.x, t7.w));
		...
		Vector4 blendResult = new Vector4();
		...
		Vector2 f = new Vector2(frac(texCoord.x * (width / scale)), frac(texCoord.y * (height / scale)));
		Vector4 res = lerp(
	...
	{
		return (int)dot(color, new Vector3(65536.0f, 256.0f, 1.0f));
	}
	protected static float distYCbCr(Vector3 pixA, Vector3 pixB)
	{
		Vector3 w = new Vector3(0.2627f, 0.6780f, 0.0593f);
	...
	protected static void scaleTexture(Texture2D tex, int scale, out Color[] scaledPixels)
	{
		...
		{
			int originY = (int)((float)i / scaledHeight * height);
			for (int j = 0; j < scaledWidth; ++j)
			{
				int originX = (int)((float)j / scaledWidth * width);
