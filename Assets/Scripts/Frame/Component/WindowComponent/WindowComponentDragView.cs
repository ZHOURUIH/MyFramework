...
{
	protected List<WindowComponentDragView> mMutexDragView; // 与当前组件互斥的滑动组件,两个组件中同一时间只有一个组件响应滑动
	...
	public void onMouseMove(ref Vector3 mousePos, ref Vector3 moveDelta, float moveTime)
	{
		// 鼠标未按下时不允许改变移动速度
		if(!mMouseDown)
		{
		...
		if(!mDraging)
		{
			...
			if(lengthGreater(ref dragDir, mDragLengthThreshold))
			{
				...
				mDraging = true;
				mOnDragViewStartCallback?.Invoke(ref mDraging);
				// 不允许拖拽
				if(!mDraging)
				{
					mMouseDown = false;
