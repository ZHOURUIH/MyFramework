...
public class CameraLinkerFree : CameraLinker
{
	...
	public override void update(float elapsedTime)
	{
		...
		if (mEnableKeyboard)
		{
			...
			{
				mMoveDelta += rotateVector3(moveLength * Vector3.forward, mCamera.getQuaternionRotation());
			}
			// 向左移动摄像机
			if (isKeyDown(KeyCode.A))
			{
				mMoveDelta += rotateVector3(moveLength * Vector3.left, mCamera.getQuaternionRotation());
			}
			// 向后移动摄像机
			if (isKeyDown(KeyCode.S))
			{
				mMoveDelta += rotateVector3(moveLength * Vector3.back, mCamera.getQuaternionRotation());
			}
			// 向右移动摄像机
			if (isKeyDown(KeyCode.D))
			{
				mMoveDelta += rotateVector3(moveLength * Vector3.right, mCamera.getQuaternionRotation());
			}
			// 竖直向上移动摄像机
