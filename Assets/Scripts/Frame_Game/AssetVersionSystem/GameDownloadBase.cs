...
{
	protected List<string> mNeedDownloadFileList = new();			// 需要下载的所有文件,StreamingAssets下的相对路径,带后缀名
	...
	protected DateTime mDownloadingTimer;							// 计时每秒通知一次下载进度
	...
	protected bool mAllFinish;										// 是否已经全部完成
	protected bool mNeedWritePersistentFileList;					// 在最后是否需要更新写入PersistDataPath中的文件列表
	...
	public void setDynamicDownloadList(List<string> list)  { mDynamicDownloadList.setRange(list.safe()); }
