...
#endif
public class MenuCheckCode : EditorCommonUtility
{
	...
	public static void checkResetProperty()
	{
		...
		doCheckResetProperty(assemly, FrameDefine.F_SCRIPTS_PATH);
		Debug.Log("检查完毕");
	}
	...
	public static void checkHotFixResetProperty()
	{
		...
		openFile(FrameDefine.F_ASSET_BUNDLE_PATH + FrameDefine.ILR_FILE, out byte[] fileContent, true);
		doCheckResetProperty(Assembly.Load(fileContent), FrameDefine.F_HOT_FIX_PATH);
		Debug.Log("检查完毕");
	}
	...
	public static void checkDifferentNodeName()
	{
		...
		findFiles(FrameDefine.F_SCRIPTS_LAYOUT_SCRIPT_PATH, fileListCS, ".cs");
		// 在热更文件夹中寻找指定文件格式的格式
