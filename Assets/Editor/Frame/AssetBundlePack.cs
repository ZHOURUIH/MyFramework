...
public class AssetBundlePack : EditorCommonUtility
{
	...
	public static void packAssetBundle(BuildTarget target, string outputPath, string subPath = EMPTY, bool showMessageBox = true)
	{
		if (isEmpty(subPath))
		{
		...
		if (!isEmpty(subPath))
		{
			if (!EditorUtility.DisplayDialog("打包", "确认打包" + subPath + "?\n部分打包仅重新生成资源包文件,清单文件也会一起生成,但是由于只是部分打包,所以依赖项可能没有打包,需要手动还原清单文件", "确认", "取消"))
			{
				return;
			}
