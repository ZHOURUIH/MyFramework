...
public class EditorMenu : EditorCommonUtility
{
	...
	public static void checkBuiltInImageAndReplace()
	{
		...
		//内置UI的相应名称
		var buildInUIName = new List<string> { "Background", "Checkmark", "DropdownArrow", "InputFieldBackground", "Knob", "UIMask", "UISprite" };
		//查找路径同时加载Prefab对象
		findFiles(FrameDefine.F_LAYOUT_PATH, filePathList, ".prefab");
		foreach (var path in filePathList)
		{
			string loadPath = fullPathToProjectPath(path);
			objectList.Add(AssetDatabase.LoadAssetAtPath<GameObject>(loadPath));
		}
		//输入日志信息
		foreach (var item in objectList)
		{
			Image[] cps = item.GetComponentsInChildren<Image>(true);
			foreach (var cp in cps)
			{
				if (cp.sprite == null || buildInUIName.Contains(cp.sprite.name))
				{
					Debug.LogError("布局 " + item.name + " 的子节点 " + cp.name + "的使用了内置的UI,请检查", item);
				}
			}
		}
	...
	public static void checkResourcesName()
	{
		...
		for (int i = 0; i < fileCount; i++)
		{
			...
			if (!endWith(fileName,".meta") && (fileName.Contains(" ") || fileName.Contains("/")))
			{
	...
	public static void checkCodeEmptyLine()
	{
		...
		for (int i = 0; i < fileCount; ++i)
		{
			...
			// 判断是否是需要忽略检查的文件
			bool isIgnore = false;
			foreach (var item in FrameDefineExtension.IGNORE_EMPTY_LINE_DIR)
			{
			...
			if (isILRuntimeGenerateFile(file))
			{
				continue;
			}
	...
	public static void checkDifferentNodeName()
	{
		...
		for (int i = 0; i < fileCount; ++i)
		{
			...
			string scriptName = removeSuffix(getFileName(filePath));
			if (!startWith(scriptName, "Script") || FrameDefineExtension.IGNORE_LAYOUT_SCRIPT.Contains(scriptName))
			{
				continue;
			}
			string fileName = removeStartString(scriptName, "Script");
			if (!prefabChildTransform.ContainsKey(fileName))
			{
				Debug.LogError("脚本名为:" + fileName + "的cs文件没找到相对应的预制体");
				continue;
			}
			openTxtFileLines(file, out string[] lines);
			...
			doCheckDifferentNodeName(file, fileName, prefabChildTransform[fileName], lines);
		}
		EditorUtility.ClearProgressBar();
	...
	public static void checkSingleCodeLineLength()
	{
		...
		for (int i = 0; i < fileCount; i++)
		{
			...
			// ILRuntime自动生成的代码和MyStringBuilder都不需要检测代码长度
			if (isILRuntimeGenerateFile(file) || 
				fileNameNoSuffix == typeof(MyStringBuilder).ToString() || 
				fileNameNoSuffix == "ToolAudio" || 
				fileNameNoSuffix == "ToolLayout" || 
				fileNameNoSuffix == "ToolFrame" || 
				fileNameNoSuffix == "ToolObject")
			{
				continue;
			}
			openTxtFileLines(file, out string[] lines);
			doCheckSingleCheckCodeLineLength(file, lines);
		}
		EditorUtility.ClearProgressBar();
		Debug.Log("检测结束");
	}
	...
	public static void checkCommentStandard()
	{
		...
		for (int i = 0; i < fileCount; i++)
		{
			...
			if (isILRuntimeGenerateFile(filePath))
			{
				continue;
			}
	...
	public static void checkCommandName()
	{
		...
		for (int i = 0; i < fileCount; i++)
		{
			...
			if (isILRuntimeGenerateFile(scriptFileList[i]))
			{
				continue;
			}
			openTxtFileLines(filePath, out string[] lines);
	...
	//---------------------------------------------------------------------------------------------------------------------------
	// 是否是ILRuntime自动生成的文件
	protected static bool isILRuntimeGenerateFile(string filePath)
	{
		return filePath.Contains("/GeneratedCLRBinding/") || filePath.Contains("/GeneratedCrossBinding/") || filePath.Contains("/ValueTypeBind/");
	}
	...
	protected static string findConstVariableName(string codeLine)
	{
		...
		int equalIndex = findFirstSubstr(codeLine, "=");
		int startIndex = -1;
		int endIndex = -1;
		for (int i = equalIndex - 1; i >= 0; --i)
		{
			if (codeLine[i] != ' ' && endIndex < 0)
			{
				endIndex = i;
			}
			if (endIndex < 0)
		...
		return codeLine.Substring(startIndex + 1, endIndex - startIndex);
	}
	...
	protected static string findMemberVariableName(string codeLine)
	{
		...
		if (index >= 0)
		{
			codeLine = codeLine.Remove(index);
		}
		...
		if (elementList.Count != 2)
		{
			...
			string typeString = stringsToString(elementList, ' ');
			// 带空格的类型都是带模板参数的类型,其他类型不考虑
			if (typeString.IndexOf('<') < 0 || typeString[typeString.Length - 1] != '>')
			{
				return null;
			}
		}
	...
	// 获取一行中的函数名,如果这一行不是函数定义返回值为空
	protected static string findFunctionName(string codeLine)
	{
		if (!isFunctionLine(codeLine))
		{
			return null;
		}
		...
		for (int i = endIndex - 1; i >= 0; --i)
		{
			if (codeLine[i] == ' ')
			{
				return codeLine.Substring(i + 1, endIndex - i - 1);
			}
		}
		return null;
	}
	...
	protected static string findClassName(string codeLine)
	{
		...
		int spaceIndex = findFirstSubstr(codeLine, " ", startIndex);
		int colonIndex = findFirstSubstr(codeLine, ":", startIndex);
		// 两个符号均未找到
	...
	protected static void doCheckCommandName(string filePath, string[] lines, Assembly csharpAssembly, Assembly hotfixAssembly)
	{
		...
		for (int i = 0; i < lines.Length; ++i)
		{
			...
			int templateIndex = findFirstSubstr(className, "<");
			// 包含'<'符号说明是泛型类
			if (templateIndex > 0)
			{
				className = className.Substring(0, templateIndex);
	...
	protected static string findBaseClassName(string codeLine)
	{
		...
		int colonIndex = findFirstSubstr(codeLine, ":");
		// 该类没有继承对象
		if (colonIndex < 0)
		{
			return null;
	...
	protected static void doCheckScriptLineByLine(string filePath, string[] lines)
	{
		...
		var ignoreArr = FrameDefineExtension.mCheckClassIgnore;
		for (int i = 0; i < lines.Length; ++i)
		{
			// 移除注释			  
			if (lines[i].Contains("//"))
			...
			{
				// 检查函数命名规范
				if (doCheckFunctionName(filePath, j, lines[j], className))
				{
					continue;
				}
				if (doCheckConstVariable(filePath, j, lines[j]))
				{
					continue;
				}
				if (doCheckEnumVariable(filePath, j, lines[j]))
				{
					continue;
				}
				doCheckNormalVariable(filePath, j, lines[j], lines[getMin(j + 1, lines.Length - 1)]);
			}
		}
	}
	// 检查常量
	protected static bool doCheckConstVariable(string filePath, int index, string codeLine)
	{
	...
	// 检查枚举
	protected static bool doCheckEnumVariable(string filePath, int index, string codeLine)
	{
	...
	protected static bool doCheckFunctionName(string filePath, int index, string codeLine, string className)
	{
		string functionName = findFunctionName(codeLine);
		// 返回值为空说明这一行不是函数声明行
		...
		if (functionName == "Reset" ||
			functionName == "Awake" ||
			functionName == "OnEnable" ||
			functionName == "Start" ||
			functionName == "FixedUpdate" ||
			functionName == "Update" ||
			functionName == "LateUpdate" ||
			functionName == "OnGUI" ||
			functionName == "OnDisable" ||
			functionName == "OnDestroy" ||
			functionName.Contains("OnTriggerEnter") ||
			functionName.Contains("OnTriggerStay") ||
			functionName.Contains("OnTriggerExit") ||
			functionName.Contains("OnCollisionEnter") ||
			functionName.Contains("OnCollisionStay") ||
			functionName.Contains("OnCollisionExit"))
		{
			return false;
		}
		// 忽略构造函数
		...
		{
			Debug.LogError("函数命名不规范" + addFileLine(filePath, index + 1));
			return true;
		}
		return false;
	}
	// 检查普通变量
	protected static bool doCheckNormalVariable(string filePath, int index, string codeLine, string nextLine)
	{
		...
		do
		{
			...
			if (normalVariable.StartsWith("m"))
			{
				if (normalVariable.StartsWith("_"))
				{
					break;
				}
				return false;
		...
		Debug.LogError("变量命名不规范" + addFileLine(filePath, index + 1));
		return true;
	}
	// 检查单行代码长度
	protected static void doCheckSingleCheckCodeLineLength(string filePath, string[] lines)
	{
		for (int i = 0; i < lines.Length; i++)
		{
			string line = lines[i];
	...
	protected static string findObjectNameWithScriptGUID(string[] lines, int scriptLineIndex)
	{
		...
		{
			if (refID == null && lines[i].Contains("&"))
			{
	...
	protected static void doCheckEmptyLine(string file, string[] lines)
	{
		...
		{
			if (isFunctionLine(lines[i]) && lines[i - 1].Length == 0)
			{
		...
		for (int i = 0; i < lines.Length; ++i)
		{
			...
			for (int j = 0; j < line.Length; ++j)
			{
				...
				if (line[j] == ' ')
				{
					if (j + 1 < line.Length && line[j + 1] == ' ')
					{
						Debug.LogError("不能有连续2个或以上的空格" + addFileLine(file, i + 1));
					}
				}
			}
		}
	...
	{
		int startIndex = findFirstSubstr(codeLine, " class ", 0, true);
		if (startIndex < 0)
		{
			return false;
		}
	...
	// 判断一行字符串是不是函数名声明行
	protected static bool isFunctionLine(string line)
	{
		// 移除注释
		...
		// 泛型方法
		if (newLine.Contains("<T>") && newLine.Contains("where T :"))
		{
			return true;
		}
		// 抽象函数没有定义
	...
	protected static void doCheckEnumComment(string filePath, string[] lines, int index, string enumName)
	{
		...
		while (true)
		{
			...
			if (lastLine.Contains("}") || lastIndex == 0 || lastLine.Length == 0)
			{
	...
	protected static void doCheckSystemFunction(string filePath, string[] lines)
	{
		...
		for (int i = 0; i < lines.Length; ++i)
		{
			...
			{
				doCheckIncludeString(lines[j], filePath, j + 1, "Debug", "DrawLine");
				doCheckIncludeString(lines[j], filePath, j + 1, "Mathf");
				doCheckIncludeString(lines[j], filePath, j + 1, "Vector3", vector3IgnoreList);
			}
		}
	}
	protected static void doCheckIncludeString(string lineString, string filePath, int index, string className, params string[] ignoreFuncionList)
	{
		// 函数名中不包含非常量的使用 即使是常量也可以忽略
		if (isFunctionLine(lineString))
		{
			return;
		}
		// 移除注释
		...
		while (true)
		{
			...
			if (firstIndex > 0 && isClassNameChar(codeLine[firstIndex - 1]))
			{
				// 移除这部分字符串
			...
			// 找到一个有效的方法
			string functionString = findFunctionString(codeLine, firstIndex + className.Length, out int resultIndex);
	...
	protected static string findFunctionString(string code, int startIndex, out int resultIndex)
	{
		resultIndex = -1;
	...
	public static bool isClassName(string str)
	{
		int length = str.Length;
		for (int i = 0; i < length; ++i)
		{
			if (!isClassNameChar(str[i]))
			{
				return false;
			}
		}
	...
	// 查询c在class命名中是否允许使用
	protected static bool isClassNameChar(char c)
	{
